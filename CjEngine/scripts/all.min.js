...
CBoosterBox.prototype.getBooster=function(){if(gameStage.player.pickupBooster){var a=[{name:"Magnet",cls:CMagnetBooster},{name:"Tablets",cls:CTabletsBooster},{name:"Health",cls:CHeartBooster},{name:"MarioStar",cls:CSupermanBooster},{name:"Double",cls:CDoubleBooster}],a=[{name:"Tablets",cls:CTabletsBooster}],b=getRand(a);if(CBooster.list)for(a=0;a<CBooster.list.length;++a)if(CObj.checkType(CBooster.list[a],b.cls)){var c=CBooster.list[a];break}for(a=0;a<PlayerData.inst.items.length&&PlayerData.inst.items[a].name!=
b.name;++a);var d=PlayerData.inst.getUpgrade(PlayerData.inst.items[a]).upgr,e=new b.cls(SCR_WIDTH/2,SCR_HEIGHT/2,null,d);e.name=b.name;e.item_id=PlayerData.inst.items[a].id;c?(a=c.x,c.duration+=e.duration,e.destroy()):(c&&(b=CBooster.list.indexOf(e),0<=b&&CBooster.list.splice(b,1)),e.gfx=crsp(PlayerData.inst.items[a].gfx),e.gfx.scale.x=.3,e.gfx.scale.y=.3,e.updateGraphics(),new TweenMax(e.gfx.scale,.3,{x:.6,y:.6,repeat:2,yoyo:!0}),a=35+50*(CBooster.list.length-1),new TweenMax(e,.7,{delay:.8,x:a,y:100}),
a=function(){e.onActivate()},c&&(a=function(){e.destroy()}),new TweenMax(e.gfx.scale,.7,{delay:.8,x:.3,y:.3,onComplete:a}),SM.inst.guiLayer.addChild(e.gfx))}};
...
MM=function(){this.patterns=[{mons:"+........",diff:1,prob:.8}];this.carClips=["car","car1","car2"];this.bosses=[{cls:Boss1,dist:1E3},{cls:Boss2,dist:2E3}];this.monY=360;this.sNormal=1;this.sBoss=2;this.lastSpawnPlane=this.lastStep=0;this.currentBoss=null;this.state=this.sNormal;this.stepDelay=330};MM.inst=new MM;
