var CG_GROUND=1,CG_MONSTER=2,CG_PLAYER=4,CG_BULLET=8;function extractName(a){return a.name?(a.name+" "+a.last_name).substring(0,16):"\u0410\u043d\u043e\u043d\u0438\u043c"}function findByProp(a,b,c){for(var d=a.length,f=0;f<d;++f)if(a[f][b]==c)return a[f];return null}function getRand(a){return a[Math.floor(Math.random()*a.length)]}function OpenInNewTab(a){window.open(a,"_blank").focus()}function clone(a){return JSON.parse(JSON.stringify(a))}
function superRand(a){for(var b=0,c=0;c<a;++c)b+=Math.random();return b/a}function dateDiff(a,b){var c=(new Date).getTime()-a.getTime(),c=(6E4*b-c)/1E3,d=Math.floor(c/3600),c=c%3600,f=Math.floor(c/60),c=c%60,g=Math.floor(c%60);return{d:c,m:f,h:d,s:g,timeString:(10>d?"0"+d:d)+" : "+(10>f?"0"+f:f)+" : "+(10>g?"0"+g:g)}}function crsp(a){var b=4<=a.length&&"."!=a.charAt(a.length-4)?".png":"";a=new PIXI.Sprite(PIXI.Texture.fromFrame(a+b));a.anchor.x=.5;a.anchor.y=.5;return a}
function rp(a){a&&a.parent&&a.parent.removeChild(a)}function extend(a,b,c){c=function(){};c.prototype=b.prototype;a.prototype=new c;a.prototype.constructor=a;a.superclass=b.prototype}function hslToRgb(a,b,c){if(0==b)c=b=a=c;else{var d=function(a,b,c){0>c&&(c+=1);1<c&&--c;return c<1/6?a+6*(b-a)*c:.5>c?b:c<2/3?a+(b-a)*(2/3-c)*6:a},f=.5>c?c(1+b):c+b-c*b,g=2*c-f;c=d(g,f,a+1/3);b=d(g,f,a);a=d(g,f,a-1/3)}return[Math.round(255*c),Math.round(255*b),Math.round(255*a)]}
rgbtoint=function(a,b,c){return 16777216+(a<<16)+(b<<8)+c};function rgbToHsl(a,b,c){a/=255;b/=255;c/=255;var d=Math.max(a,b,c),f=Math.min(a,b,c),g,h=(d+f)/2;if(d==f)g=f=0;else{var l=d-f,f=.5<h?l/(2-d-f):l/(d+f);switch(d){case a:g=(b-c)/l+(b<c?6:0);break;case b:g=(c-a)/l+2;break;case c:g=(a-b)/l+4}g/=6}return[g,f,h]}
var ZSound={available:!1,loaded:!1,Init:function(a){ZSound.available=createjs.Sound.initializeDefaultPlugins();ZSound.available&&(ZSound.loaded=0,ZSound.total=a.length,createjs.Sound.addEventListener("fileload",function(){ZSound.loaded++;ZSound.loaded==a.length&&ZSound.soundLoadedFunction&&(ZSound.loaded=!0,ZSound.soundLoadedFunction())}),createjs.Sound.registerManifest(a,"res/snd/"))},PlayMusic:function(a){ZSound.loaded?ZSound.PlayMusicInner(a):soundLoadedFunction=function(){ZSound.PlayMusicInner(a);
ZSound.loaded=!0}},UnMute:function(){ZSound.available||(ZSound.available=!0,createjs.Sound.setMute(!1))},Mute:function(){ZSound.available=!1;createjs.Sound.setMute(!0)},PlayMusicInner:function(a){if(ZSound.musicInstanceName!=a||!ZSound.musicInstance){ZSound.musicInstance&&createjs.Sound.stop(ZSound.musicInstance);try{ZSound.musicInstanceName=a,ZSound.musicInstance=createjs.Sound.play(a,{interrupt:createjs.Sound.INTERRUPT_NONE,loop:999999})}catch(b){}}},Play:function(a){if(ZSound.available)try{createjs.Sound.play(a,
createjs.Sound.INTERRUPT_NONE,0,0,!1,1)}catch(b){}}};LevelManager=function(){};LevelManager.onComplete=function(){LevelManager.layer||(LevelManager.layer=SM.inst.ol);CObj.AssignTexturesToObjects(LevelManager.objs,LevelManager.layer);for(var a=0;a<LevelManager.objs.length;++a)LevelManager.objs[a].init();if(LevelManager.onVeryComplete)LevelManager.onVeryComplete()};
LevelManager.removeLastLevel=function(){for(;0<LevelManager.objs.length;)LevelManager.objs[0].destroy(),LevelManager.objs.splice(0,1);objs=null};
...
null;this.transStart=0;this.transTime=600;this.doTrans=!1;$(window).mousewheel(function(a,b,c,d){if(CObj.objects)for(a=0;a<CObj.objects.length;++a)if(CObj.checkType(CObj.objects[a],CScrollbar),CObj.objects[a].mover)CObj.objects[a].onWheel(e,b,c,d);e.stopPropagation();e.stopImmediatePropagation();e.preventDefault()})}SM.inst=new SM;
...
return c};SM.prototype.addLayersToStage=function(){this.superStage.addChild(this.bg);this.superStage.addChild(this.ol);this.superStage.addChild(this.fg);this.superStage.addChild(this.guiLayer);this.superStage.addChild(this.superGuiLayer);this.superStage.addChild(this.fontLayer);window.stage.addChild(this.superStage);this.bg.mousemove=function(a){window.mouseX=a.global.x/SCR_SCALE;window.mouseY=a.global.y/SCR_SCALE}};
...
c.textField.visible=!1,c.lock.visible=!0);var d=new CObj(c.x-32,c.y+22);d.gfx=CObj.createMovieClip("star0001");d.gfx.interactive=!1;d.gfx.scale.x=.4*window.addScale;d.gfx.scale.y=.4*window.addScale;d.gfx.anchor.x=.5;d.gfx.anchor.x=.5;SM.inst.guiLayer.addChild(d.gfx);var f=new CObj(c.x,c.y+22);f.gfx=CObj.createMovieClip("star0001");f.gfx.interactive=!1;f.gfx.scale.x=.4*window.addScale;f.gfx.scale.y=.4*window.addScale;f.gfx.anchor.x=.5;f.gfx.anchor.x=.5;SM.inst.guiLayer.addChild(f.gfx);c=new CObj(c.x+
32,c.y+22);c.gfx=CObj.createMovieClip("star0001");c.gfx.interactive=!1;c.gfx.scale.x=.4*window.addScale;c.gfx.scale.y=.4*window.addScale;c.gfx.anchor.x=.5;c.gfx.anchor.x=.5;SM.inst.guiLayer.addChild(c.gfx);var g=dataStorage["lvlst"+b.toString()];g||(g=0);1==g&&d.gfx.gotoAndStop(1);2==g&&(d.gfx.gotoAndStop(1),f.gfx.gotoAndStop(1));3==g&&(d.gfx.gotoAndStop(1),f.gfx.gotoAndStop(1),c.gfx.gotoAndStop(1))}};function levelClick(a){gameStage.currentLevel=a.target.levelNum;SM.inst.openStage(gameStage)}
...
d}};var d=a.gfx.scale.x,f=a.gfx.scale.y;a.gfx.mouseover=function(b){TweenMax.killTweensOf(a.gfx.scale);a.gfx.tint=CButton.tintColor;new TweenMax(a.gfx.scale,.6,{y:f+.05,ease:Elastic.easeOut});new TweenMax(a.gfx.scale,.4,{x:d+.05,ease:Elastic.easeOut})};a.gfx.mouseout=function(b){a.gfx.tint=16777215;new TweenMax(a.gfx.scale,.3,{x:d,y:f,ease:Elastic.easeOut})};var g=new PIXI.Sprite(PIXI.Texture.fromFrame("bodrost star.png"));g.anchor.x=.5;g.anchor.y=.5;g.x=SCR_WIDTH/2;g.y=SCR_HEIGHT/2+120;SM.inst.guiLayer.addChild(g);
var h=CTextField.createTextField({text:gameStage.revealPrice.toString(),fontSize:22,align:"center"});h.tint=3355443;h.updateText();h.x=8+SCR_WIDTH/2;h.y=SCR_HEIGHT/2+107;SM.inst.guiLayer.addChild(h)}};
...
{owner_id:c.vkapi,message:"\u042f \u043f\u0440\u043e\u0435\u0445\u0430\u043b "+b.toString()+" \u043c\u0435\u0442\u0440\u043e\u0432. "+c.name+" "+c.last_name+",  \u043d\u0438\u043a\u043e\u0433\u0434\u0430 \u043d\u0435 \u043f\u043e\u0431\u044c\u0435\u0448\u044c \u043c\u043e\u0439 \u0440\u0435\u043a\u043e\u0440\u0434!!\nhttps://vk.com/app4654201",attachments:["photo282617259_360326329","https://vk.com/app4654201"]},function(a){})}}(c,a[c]));for(a=c;5>a;++a)CObj.getById("b"+(a+1).toString()).gfx.visible=
...
CharStage.prototype.onShow=function(){this.doProcess=this.unreadActions=this.unreadAch=!1;charStage.skipFriends=0;CustomStage.prototype.onShow.call(this);charStage.icons=[];ZSound.PlayMusic("m_room");LevelManager.loadLevel("levchar",function(){LevelManager.loadLevel("upperPanel",charStage.onShowContinue,SM.inst.ol)},SM.inst.ol)};
...
CharStage.prototype.createFriendsPanel=function(){for(var a=new PIXI.DisplayObjectContainer,b="",c=[],d=charStage.skipFriends,f=0+d;f<5+d;++f){var g=new CButton(91+95*(f-d),5,"add friend"),h=g.gfx;h.parent.removeChild(h);g.fontFamily="dedgamedesc";g.fontSize=14;g.tint=16777215;h.anchor.x=.5;h.anchor.y=.5;g.hover=!0;g.deltaHoverY=17;vkparams.friendsIngame&&f<vkparams.friendsIngame.length&&(g.text=vkparams.friendsIngame[f].name+" "+vkparams.friendsIngame[f].last_name);g.addToSameLayer=!0;g.init();a.addChild(h);
!vkparams.friendsIngame||f>=vkparams.friendsIngame.length?h.click=function(){VK.callMethod("showInviteBox")}:(function(a,b,c){console.log(a.toString()+" / "+b.toString());c.click=function(){VK.api("wall.post",{owner_id:b,message:a+", \u0432\u043e\u0437\u0432\u0440\u0430\u0449\u0430\u0439\u0441\u044f \u0432 \u0438\u0433\u0440\u0443, \u0434\u0440\u0443\u0436\u0438\u0449\u0435\nhttps://vk.com/app4654201",attachments:["photo282617259_360326325","https://vk.com/app4654201"]},function(a){})}}(vkparams.friendsIngame[f].name,
vkparams.friendsIngame[f].vkapi,h),""!=b&&(b+=","),b+=vkparams.friendsIngame[f].vkapi.toString(),h.vkapi=vkparams.friendsIngame[f].vkapi,c.push(h))}VK&&""!=b&&VK.api("users.get",{user_ids:b,fields:"photo"},function(a){if(a.response)for(var b=0;b<a.response.length;++b){var d=a.response[b].photo;if(!a.response||0==a.response.length)break;var f=c[b];f.loader=new PIXI.ImageLoader(d);(function(a,b){a.loader.onLoaded=function(){var c=new PIXI.Sprite(PIXI.TextureCache[b]);c.anchor.x=.5;c.anchor.y=.5;a.addChild(c);
charStage.icons.push(c)}})(f,d);f.loader.load()}});a.x=215;a.y=SCR_HEIGHT-50;SM.inst.guiLayer.addChild(a);return a};
CharStage.prototype.openPremiumWindow=function(){CObj.enableButtons(!1);var a=SM.inst.addDisableWindow(null,SM.inst.fontLayer);LevelManager.loadLevel("levelpremium",function(){close=function(){LevelManager.removeLastLevel();CObj.enableButtons(!0);a.parent.removeChild(a)};charStage.skipFriends=0;orderFunc=function(a){ZSound.Play("buy");order(a);shopStage.updateStatsPanel()};CObj.getById("btnfree").click=function(){VK.callMethod("showOrderBox",{type:"offers",currency:"true"})};CObj.getById("buy1").click=
function(){orderFunc("item1")};CObj.getById("buy2").click=function(){orderFunc("item2")};CObj.getById("buy3").click=function(){orderFunc("item3")};CObj.getById("buy4").click=function(){orderFunc("item4")};CObj.getById("buy5").click=function(){orderFunc("item5")};CObj.getById("buy6").click=function(){orderFunc("item6")};CObj.getById("btnclose").click=close},SM.inst.fontLayer)};
CharStage.prototype.openEnergyWindow=function(){CObj.enableButtons(!1);var a=SM.inst.addDisableWindow(null,SM.inst.fontLayer);a.interactive=!0;LevelManager.loadLevel("energywindow",function(){CObj.getById("breplenish").click=function(){order("item7");VK.orderComplete=function(a){shopStage.updateStatsPanel();close()}};close=function(){rp(a);CObj.enableButtons(!0);LevelManager.removeLastLevel();a.click=null};a.click=function(){var a=CObj.getById("energybg");a.gfx.getBounds();window.mouseX>a.x-a.gfx.width/
2&&window.mouseX<a.x+a.gfx.width/2&&window.mouseY>a.y-a.gfx.height/2&&window.mouseY<a.y+a.gfx.height/2||close()}},SM.inst.fontLayer)};CharStage.prototype.updateMusicButton=function(a){ZSound.available?a.gfx.gotoAndStop(0):a.gfx.gotoAndStop(1);a.click=function(){ZSound.available?ZSound.Mute():ZSound.UnMute();ZSound.available?a.gfx.gotoAndStop(0):a.gfx.gotoAndStop(1)}};
...
CEActionGUI)&&CObj.objects[b].acting){var c=(window.time-CObj.objects[b].startTime)/CObj.objects[b].execTime;1<=c?(CObj.objects[b].pos=1,CObj.objects[b].endAction(),this.unreadActions=!0,this.updateNotifications()):CObj.objects[b].pos=c;a++}};
...
CharStage.prototype.openScore=function(){CObj.enableButtons(!1);var a=SM.inst.addDisableWindow(null,SM.inst.fontLayer);LevelManager.loadLevel("levscore",function(){charStage.container=new PIXI.DisplayObjectContainer;charStage.container.x=170;charStage.container.y=170;SM.inst.fontLayer.addChild(charStage.container);var b="0";PlayerData.inst.playerItem.maxdistance&&(b=PlayerData.inst.playerItem.maxdistance.toString());var c="";PlayerData.inst.playerItem.rank&&(c=PlayerData.inst.playerItem.rank.toString()+
".");CObj.getById("tplscore").text=b+"\u043c.";CObj.getById("tplace").text=c;CObj.getById("bfriends").click=function(){charStage.skip=0;"friends"==charStage.tab?(CObj.getById("bfriends").text="\u0412\u0441\u0435 \u043e\u0447\u043a\u0438",charStage.tab="total",charStage.updateTotal()):(CObj.getById("bfriends").text="\u041e\u0447\u043a\u0438 \u0434\u0440\u0443\u0437\u0435\u0439",charStage.tab="friends",charStage.updateFriends())};charStage.tab="friends";CObj.getById("bfriends").click();CObj.getById("bforwardlist").click=
...
charStage.tab&&charStage.updateTotal();"friends"==charStage.tab&&charStage.updateFriends()}},SM.inst.fontLayer)};
CharStage.prototype.updateSB=function(a){for(;0<charStage.container.children.length;)rp(charStage.container.getChildAt(0));for(var b=[],c="",d=0;d<a.length;++d){var f=new PIXI.DisplayObjectContainer,g=CTextField.createTextField({tint:3355443,fontSize:17,text:(charStage.skip+d+1).toString()+"."}),h=CTextField.createTextField({tint:3355443,fontSize:17,text:extractName(a[d])}),l=CTextField.createTextField({tint:3355443,fontSize:17,text:Math.round(a[d].maxdistance).toString()+"\u043c."}),k=crsp("ava cover");
b.push(k);b[d].vkapi=a[d].vkapi;""!=c&&(c+=",");c+=a[d].vkapi;if(3>charStage.skip+d){var m;0==charStage.skip+d&&(m="1st place");1==charStage.skip+d&&(m="2nd place");2==charStage.skip+d&&(m="3rd place");var n=crsp(m);n.x=4;n.y=10;n.scale.x=.8;n.scale.y=.8;f.addChild(n)}k.y=10;k.x=35;f.x=20;f.y=15+28*d;h.x=55;l.x=370;f.addChild(k);f.addChild(h);f.addChild(g);f.addChild(l);charStage.container.addChild(f)}VK&&""!=c&&VK.api("users.get",{user_ids:c,fields:"photo"},function(a){if(a.response)for(var c=0;c<
a.response.length;++c){var d=a.response[c].photo;if(!a.response||0==a.response.length)break;var f=b[c];f.loader=new PIXI.ImageLoader(d);(function(a,b){a.loader.onLoaded=function(){var c=new PIXI.Sprite(PIXI.TextureCache[b]);c.scale.x=.5;c.scale.y=.5;c.anchor.x=.5;c.anchor.y=.5;a.addChild(c)}})(f,d);f.loader.load()}})};function ShopStage(){CustomStage.apply(this)}extend(ShopStage,CustomStage);
...
ShopStage.prototype.createItemBtn=function(a){function b(a){shopStage.buyItem(a,!0)}function c(b){PlayerData.inst.equipItem(a);shopStage.pl.updateAppearence(!0,!1,null,null,null);shopStage.updateBar(shopStage.currentTab,shopStage.currentFilter,shopStage.bar.pos)}function d(a){shopStage.buyItem(a,!1)}var f=new PIXI.DisplayObjectContainer,g="bstuff"==shopStage.currentTab;if(g){var h=PlayerData.inst.getUpgrade(a),l=h.upgr;if(h.idNext)a=PlayerData.inst.getItemById(h.idNext);else{var k=!0;a=PlayerData.inst.getItemById(h.id)}}var m=
crsp(a.gfx);m.x=0;m.scale.x=.5;m.scale.y=.5;m.y=m.height/2;f.addChild(m);m.interactive=!0;m.mouseover=function(b){if(a.type==tWeapon||a.type==tApp+tHat){var c=b=null;a.type==tWeapon&&(c=a.gfx);a.type==tApp+tHat&&(b=a.gfx);shopStage.pl.updateAppearence(!0,!1,null,c,b)}TweenMax.killTweensOf(m.scale);m.tint=CButton.tintColor;new TweenMax(m.scale,.6,{y:.55,ease:Elastic.easeOut});new TweenMax(m.scale,.4,{x:.55,ease:Elastic.easeOut});c=a.desc.split("|");b=c[0];1<c.length&&(b=c[1]);c=CObj.getById("tfdescd");
c.setTextSafe(b);c.item=a};m.mouseout=function(b){b=CObj.getById("tfdescd");b.item==a&&b.setTextSafe("");a.type!=tWeapon&&a.type!=tApp+tHat||shopStage.pl.updateAppearence(!0,!1,null,null,null);m.tint=16777215;m.currentFrame&&m.gotoAndStop(1);new TweenMax(m.scale,.3,{x:.5,y:.5,ease:Elastic.easeOut})};if(g)for(var n=0;5>n;++n){var p=n<l?crsp("improve point on"):crsp("improve point off");p.x=25*(n-2);p.scale.x=.8;p.scale.y=.8;p.y=3;f.addChild(p)}n=this.checkOwned(a);p=this.checkEq(a);tftext="";g&&(k?
tftext="":n||(tftext=0>a.pricecrys&&0>a.price?"":0<a.pricecrys?a.pricecrys.toString():a.price.toString()));var r,k="",q="buy button",t;!n||g?g&&5==l?q="equipped button":a.reqlvl>PlayerData.inst.playerItem.lvl?(r=b,q="unlock button",k="\u041c\u0418\u041d "+a.reqlvl.toString()+" \u0423\u0420.",t="price star.png",tftext=a.pricecrys.toString()):(r=d,t="price coin.png",q="buy button",tftext=a.price.toString()):1==p?q="equipped button":(r=c,q="wear button");l=new CButton(0,129,q);"equipped button"==q&&
(l.gfx.mouseout=null,l.gfx.mouseover=null);l.fontSize=33;l.addToSameLayer=!0;l.gfx.anchor.x=.5;l.gfx.anchor.y=.5;l.gfx.scale.x=.8;l.gfx.scale.y=.8;l.init();l.gfx.parent.removeChild(l.gfx);l.item=g?PlayerData.inst.getItemById(h.idNext):a;l.gfx.btn=l;l.click=r;f.addChild(l.gfx);l.text&&(l.text=l.text);f.btn=l;tftext&&t&&(g=crsp(t),g.x=0,g.y=80,g.scale.x=.75,g.scale.y=.75,f.addChild(g));g=new CTextField;g.tint="0x333333";g.fontSize=15;g.align="center";g.init();f.infoTF=g;g.x=0;g.y=158;g.gfx.parent.removeChild(g.gfx);
g.text=k;f.addChild(g.gfx);g=new CTextField;g.tint="0x333333";g.fontSize=15;g.align="center";g.text=tftext;g.init();f.tfprice=g;g.x=10;g.y=70;g.gfx.parent.removeChild(g.gfx);f.addChild(g.gfx);g.text=g.text;f.destroy=function(){f.tfprice.destroy();f.btn&&(f.btn.destroy(),f.btn=null);f.tfprice=null;f.parent.removeChild(f)};return f};
ShopStage.prototype.updateBar=function(a,b,c){shopStage.updateStatsPanel();shopStage.currentTab=a;shopStage.currentFilter=b;CObj.getById("bstuff").gfx.alpha=.1;CObj.getById("bweap").gfx.alpha=.1;CObj.getById("bcloth").gfx.alpha=.1;CObj.getById(a).gfx.alpha=1;for(var d=0;d<this.bar.container.children.length;++d)this.bar.container.children[d].destroy(),d--;var f=2;"bweap"==a&&(f=2);a=this.bar.gfx.width/f-6;for(var g=0,d=0;d<PlayerData.inst.items.length;++d)if(!(0>PlayerData.inst.items[d].type.indexOf(b)||
0<=PlayerData.inst.items[d].name.indexOf("$"))){var h=shopStage.createItemBtn(PlayerData.inst.items[d]);h.x=a/2+g%f*a;h.y=40+195*Math.floor(g/f);this.bar.container.addChild(h);g++}c||(c=0);this.bar.pos=c};
...
ShopStage.prototype.process=function(){var a=CObj.getById("tfdelay");if(a&&1>PlayerData.inst.playerItem.energy)a.gfx.visible=!0;else if(a.gfx.visible=!1,Math.round(PlayerData.inst.playerItem.energy)<Math.round(PlayerData.inst.maxEnergy)){var b=dateDiff(PlayerData.inst.playerItem.updateDate,PlayerData.inst.delayEnergyMS/6E4);a.text=b.timeString}else a.text="";CObj.processAll()};function ScoreStage(){CustomStage.apply(this)}extend(ScoreStage,CustomStage);
...
ScoreStage.prototype.updateSB=function(a){for(;0<scoreStage.container.children.length;)rp(scoreStage.container.getChildAt(0));for(var b=[],c="",d=0;d<a.length;++d){var f=new PIXI.DisplayObjectContainer,g=CTextField.createTextField({tint:3355443,fontSize:15,text:(scoreStage.skip+d+1).toString()+"."}),h=CTextField.createTextField({tint:3355443,fontSize:15,text:a[d].name+" "+a[d].last_name}),l=CTextField.createTextField({tint:3355443,fontSize:15,text:Math.round(a[d].maxdistance).toString()+"\u043c."}),
k=crsp("buy small button");b.push(k);b[d].vkapi=a[d].vkapi;""!=c&&(c+=",");c+=a[d].vkapi;if(3>scoreStage.skip+d){var m;0==scoreStage.skip+d&&(m="1st place");1==scoreStage.skip+d&&(m="2nd place");2==scoreStage.skip+d&&(m="3rd place");var n=crsp(m);n.x=4;n.y=10;n.scale.x=.8;n.scale.y=.8;f.addChild(n)}k.y=10;k.x=35;f.x=20;f.y=15+28*d;h.x=55;l.x=370;f.addChild(k);f.addChild(h);f.addChild(g);f.addChild(l);scoreStage.container.addChild(f)}VK&&""!=c&&VK.api("users.get",{user_ids:c,fields:"photo"},function(a){for(var c=
0;c<a.response.length;++c){var d=a.response[c].photo;if(!a.response||0==a.response.length)break;var f=b[c];f.loader=new PIXI.ImageLoader(d);(function(a,b){a.loader.onLoaded=function(){var c=new PIXI.Sprite(PIXI.TextureCache[b]);c.scale.x=.5;c.scale.y=.5;c.anchor.x=.5;c.anchor.y=.5;a.addChild(c)}})(f,d);f.loader.load()}})};
...
ComixStage.prototype.goNext=function(){comixStage.dcNext&&comixStage.dcNext.kill();TweenMax.killAll(!0,!0,!0);var a=this.comixData.shift();a||SM.inst.openStage(gameStage);for(var b=0;b<a.objects.length;++b){var c=a.objects[b],d=crsp(c.tex);comixStage.comixContainer.addChild(d);d.visible=!1;c.rot1||(c.rot1=0);c.rot2||(c.rot2=0);c.p1||(c.p1=[0,0]);c.p2||(c.p2=c.p1);c.s1||(c.s1=[1,1]);c.s2||(c.s2=c.s1);d.scale.x=1.1*c.s1[0];d.scale.y=1.1*c.s1[1];d.rotation=c.rot1;d.x=c.p1[0];d.y=c.p1[1];d.vx=0;d.vy=
...
ZPool=function(){this.objects={};ZPool.Inst=this;return this};ZPool.prototype.Fill=function(a,b,c){void 0==this.objects[a]&&(this.objects[a]=[]);for(var d=0;d<b;d++){var f=c();f.poolName=a;this.objects[a].push(f)}};ZPool.prototype.Pop=function(a){if(void 0==this.objects[a])console.warn("Error: not found '"+a+"' in pool.");else if(0==this.objects[a].length)console.warn("Error: pool empty. Element '"+a+"'");else return this.objects[a].pop()};
...
CObj.processAll=function(){for(var a=CObj.objects.length,b=0;b<a;b++)if(!CObj.objects[b].gui&&(CObj.objects[b].x>1.8*SCR_WIDTH||CObj.objects[b].x<.8*-SCR_WIDTH||CObj.objects[b].y>1.8*SCR_HEIGHT||CObj.objects[b].y<-SCR_HEIGHT))CObj.checkType(CObj.objects[b],CHPBar)||CObj.objects[b].destroy();else if(0<CObj.objects[b].colMask)for(var c=0;c<a;c++)if(b!=c){var d=CObj.objects[b],f=CObj.objects[c];if(!(d.doRemove||f.doRemove||0==(d.colMask&f.colGroup)&&0==(f.colMask&d.colGroup)))if(d.hitTestCircles)for(var g=
d.hitTestCircles.length,h=0;h<g;++h){var l=f.x-(d.x+d.hitTestCircles[h].x),k=f.y-(d.y+d.hitTestCircles[h].y),m=f.radius+d.hitTestCircles[h].r;if(l*l+k*k<m*m){0!=(d.colMask&f.colGroup)&&d.collide(f);0!=(f.colMask&d.colGroup)&&f.collide(d);break}}else l=d.x-f.x,k=d.y-f.y,l*l+k*k<(f.radius+d.radius)*(f.radius+d.radius)&&(0!=(d.colMask&f.colGroup)&&d.collide(f),0!=(f.colMask&d.colGroup)&&f.collide(d))}for(b=0;b<a;b++)CObj.objects[b].process(),CObj.objects[b].doRemove&&(CObj.objects[b]._destroy(),CObj.objects.splice(b,
...
CObj.ExtractFrameNum=function(a){var b=a.indexOf("0");if(-1==b)return 0;a=a.substring(b,a.length);~a.indexOf(".")&&(a=a.slice(0,-4));a=parseInt(a);return isNaN(a)?0:a};CObj.createMovieClip=function(a){var b=CObj.ExtractFrameName(a),c=[],d=0,f=0,g;for(g in PIXI.TextureCache)a==g&&(d=f),CObj.ExtractFrameName(g)==b&&(c.push(PIXI.TextureCache[g]),f++);0==c.length&&(c[0]=PIXI.TextureCache[a+".png"]);a=new PIXI.MovieClip(c);a.gotoAndStop(d);return a};
CObj.AssignTexturesToObjects=function(a,b){for(var c=a.length,d=0;d<c;d++)if(""!=a[d].clipSrc&&null!=a[d].clipSrc){var f=null;""!=a[d].sAtlasName?f=PIXI.Texture.fromFrame(a[d].clipSrc+".png"):(f=a[d].clipSrc.indexOf("."),f=-1==f?PIXI.Texture.fromFrame(a[d].clipSrc+".png"):PIXI.Texture.fromFrame(a[d].clipSrc.substr(0,f)+".png"));if(f){var g=null;a[d].isClip?(g=CObj.createMovieClip(a[d].clipSrc),g.animationSpeed=a[d].fps/FRAME_RATE,a[d].autoPlay&&g.play()):g=a[d].drawAsTexture?new PIXI.TilingSprite(f,
a[d].baseDim.x,a[d].baseDim.y):new PIXI.Sprite(f);g.anchor.x=.5;g.anchor.y=.5;g.x=a[d].x;g.y=a[d].y;g.width=a[d].baseDim.x+2;g.height=a[d].baseDim.y+2;g.rotation=a[d].rotation;g.scale.x=a[d].scaleX*window.addScale;g.scale.y=a[d].scaleY*window.addScale;g.width+=1;g.height+=1;a[d].gfx=g;b&&b.addChild(g)}}};CObj.setBodyMass=function(a,b){var c=a.getArea();a.mass=c*b/50};
...
CObj.DeserializeCObj=function(a,b){var c=Deserialize(a);c.clipSrc=a.clip;if(c.clipSrc){var d=a.clip,f=d.lastIndexOf(".");-1!=d.lastIndexOf(".",f-1)&&(d.substring(0,f),c.sAtlasName=d.substring(0,f),c.clipSrc=d.substring(f+1,d.length),f=c.sAtlasName.indexOf("."),-1!=f&&(c.sAtlasName=c.sAtlasName.substr(0,f)))}void 0!=a.density&&(c.density=a.density);void 0!=a.elasticity&&(c.elasticity=a.elasticity);void 0!=a.friction&&(c.friction=a.friction);void 0!=a.frictionStatic&&(c.frictionStatic=a.frictionStatic);
...
function Deserialize(a){if(void 0!=a.cls&&""!=a.cls&&"ZEngine::ZObj"!=a.cls){var b=a.cls;-1!=b.indexOf("::")&&(b=b.split("::")[1]);window[b]?("ZObj"==b&&(b="CObj"),o=new window[b],o.cls=b):o=new CObj}else o=new CObj;if(o.PublicFields){var b=o.PublicFields.replace(" ","").split(";"),c,d,f=b.length;for(d=0;d<f;d++)if(""!=b[d]){var g=b[d].split(","),h=g.length;for(c=0;c<h;c++){var l=g[c];""!=l&&"["!=l.substr(0,1)&&void 0!=a[l]&&(o[l]=a[l])}}}return o}extend(CBomb,CObj,!0);
function CBomb(a,b,c,d){CObj.apply(this,[a,b,c,d]);this.PublicFields+="power,";this.radiusMax=300}
CBomb.prototype.explode=function(){if(!this.doRemove){var a=CObj.objects.length,b=new vec2.create,c=new vec2.create;ZSound.Play("rocketboom");if(pool.Pop("expl")){var d=new CObj(this.x,this.y);d.x=this.x;d.y=this.y;d.gfx.animationSpeed=.85;d.gfx.gotoAndPlay(0);d.gfx.loop=!1;d.gfx.scale.x=1.55*window.addScale;d.gfx.scale.y=1.55*window.addScale;d.updateGraphics();d.gfx.parent&&d.gfx.parent.removeChild(d.gfx);SM.inst.ol.addChild(d.gfx);d.gfx.onComplete=function(){d.gfx&&(d.destroy(),pool.Push(d.gfx))}}for(var f=
0;f<a;++f)if(!CObj.objects[f].sensor&&CObj.objects[f]!=this&&CObj.objects[f].body&&CObj.objects[f].body.type==p2.Body.DYNAMIC){b[0]=this.x;b[1]=this.y;c[0]=CObj.objects[f].x;c[1]=CObj.objects[f].y;vec2.sub(b,b,c);var g=vec2.sqrLen(b);g>this.radiusMax*this.radiusMax||(g=Math.sqrt(g),b[0]/=g,b[1]/=g,g=1-(g+2)/this.radiusMax,0>g&&(g=0),g=Math.pow(g,1.4),vec2.scale(b,b,4500*this.power*g),CObj.objects[f].body.force[0]-=b[0],CObj.objects[f].body.force[1]-=b[1])}this.destroy()}};
...
0);this.text&&(this.text=this.text);this.isClip&&this.gfx.gotoAndStop(0);var a=this.textField,b=this.gfx;this.baseScaleX=b.scale.x;this.baseScaleY=b.scale.y;var c=this.baseScaleX,d=this.baseScaleY,f=this;this.updateGraphics();this.gfx.mouseover=function(g){TweenMax.killTweensOf(b.scale);f.over=!0;b.tint=CButton.tintColor;new TweenMax(b.scale,.6,{y:d+.05,ease:Elastic.easeOut});new TweenMax(b.scale,.4,{x:c+.05,ease:Elastic.easeOut});a.visible&&""!=a.text&&(new TweenMax(a.scale,.6,{y:1.1,ease:Elastic.easeOut}),
new TweenMax(a.scale,.4,{x:1.1,ease:Elastic.easeOut}));f.hover&&(f.textField.alpha=1)};this.gfx.mouseout=function(g){f.over=!1;f.hover&&(f.textField.alpha=0);b.tint=16777215;b.currentFrame&&b.gotoAndStop(1);new TweenMax(b.scale,.3,{x:c,y:d,ease:Elastic.easeOut});a.visible&&""!=a.text&&new TweenMax(a.scale,.3,{x:1,y:1,ease:Elastic.easeOut})};this.addToSameLayer?this.gfx.addChild(this.textField):SM.inst.fontLayer.addChild(this.textField)};extend(ZCJoint,CObj,!0);
...
(c=d);this.gfx.skeleton.setAttachment("gun",c);f&&(b=f);this.gfx.skeleton.setAttachment("hat",b)};
...
function CBullet(a,b,c,d,f){CObj.apply(this,[a,b,null,d]);this.gfx=new PIXI.DisplayObjectContainer;this.visualVel=7;this.visualWidth=f;this.bhead=new PIXI.Sprite(PIXI.Texture.fromFrame("head.png"));this.bhead.anchor.x=.5;this.bhead.anchor.y=.5;this.bhead.blendMode=PIXI.blendModes.ADD;this.bhead.width=this.visualWidth;this.bmiddle=new PIXI.Sprite(PIXI.Texture.fromFrame("center.png"));this.bmiddle.width=this.visualWidth;this.bmiddle.anchor.x=.5;this.bmiddle.anchor.y=.5;this.bmiddle.blendMode=PIXI.blendModes.ADD;
...
LauncherBG.prototype.spawnClip=function(a,b,c,d,f){var g=CObj.DeserializeCObj(b);CObj.AssignTexturesToObjects([g],SM.inst.bg);(d=g.gfx)&&d.parent&&d.parent.removeChild(d);g.gfx=null;g.destroy();g=30;0==a.velocity&&(g=0);d?(a.clip.addChild(d),a.rightBound+=b.baseDim.x*b.scaleX,d.position.x=c?g+b.baseDim.x*b.scaleX/2:SCR_WIDTH+g+b.baseDim.x*b.scaleX/2+f-.8*this.maxVelocity-3):a.rightBound+=b.baseDim.x*b.scaleX};
...
(this.maxVelocity+=this.speedUpCoef,this.speedUpCoef*=.95,console.log("SPEED "+this.maxVelocity.toString())));MM.inst.currentBoss||(this.distance+=b);a=this.levCycles[0].layers.length;for(b=0;b<a;++b){var c=this.levCycles[0].layers[b];c.curDist+=c.velocity;c.rightBound-=c.velocity;for(var d=0;d<c.clip.children.length;++d){var f=c.clip.children[d];f.x-=c.velocity}for(d=0;d<c.clip.children.length;++d)f=c.clip.children[d],f.position.x+.5*f.width<this.x&&c.clip.removeChild(c.clip.getChildAt(d));for(var f=
c.minx+c.curDist-Math.floor(c.curDist/c.width)*c.width,g=f-c.velocity,d=0;d<c.objects.length;++d){var h=c.objects[d],l=h.x-h.scaleX*h.baseDim.x/2;l>=g&&l<f&&this.spawnClip(c,h,null,f,l-g)}if(4==b&&this.graves)for(d=this.graves.length,f=0;f<d;++f)this.distance>this.graves[f].dist-(SCR_WIDTH+100)*this.pixToDist&&(g=crsp("grave"),g.x=SCR_WIDTH+100,g.y=395,c.clip.addChild(g),h=CTextField.createTextField({fontFamily:"dedgamecaps",tint:"0xFFFFFFFF",text:this.graves[f].text+"\n"+this.graves[f].dist.toString()+
" \u043c.",fontSize:30,align:"center"}),h.x=-h.width/2,h.updateText(),g.addChild(h),this.graves.splice(f,1),f--,d--)}}};
LauncherBG.prototype.addLevel=function(a,b){var c=LevelManager.levels["levels/"+a+".json"];clone(c);for(var d=[],f=0;7>f;++f){var g=new PIXI.DisplayObjectContainer,g={rightBound:SCR_WIDTH,clip:g,curDist:SCR_WIDTH,objects:[],velocity:this.nullSpeed*this.layersSpeed[f]};d.push(g);this.gfx.addChild(g.clip)}this.gfx.addChildAt(this.planeLayer,1);this.gfx.addChildAt(this.ol,7);this.gfx.addChildAt(this.pllayer,8);for(f=0;f<c.objects.length;++f)d[c.objects[f].layer-1].objects.push(c.objects[f]);for(c=0;c<
d.length;++c){for(var f=1E5,g=-1E5,h=0;h<d[c].objects.length;++h){var l=d[c].objects[h].x-d[c].objects[h].scaleX*d[c].objects[h].baseDim.x/2,k=d[c].objects[h].x+d[c].objects[h].scaleX*d[c].objects[h].baseDim.x/2;l<f&&(f=l);k>g&&(g=k)}d[c].maxx=g;d[c].minx=f;d[c].width=g-f;if(0==d[c].velocity)for(f=0;f<d[c].objects.length;++f)this.spawnClip(d[c],d[c].objects[f],!0)}this.defaultLayer=d[d.length-1].clip;this.levCycles.push({layers:d,dist:b});this.maxVelocity=this.nullSpeed};extend(CGrenade,CBullet,!0);
function CGrenade(a,b,c,d){CObj.apply(this,[a,b,null,d]);this.gfx=crsp("Granata.png");this.gfx.scale.x=.5;this.gfx.scale.y=.5;LauncherBG.inst.ol.addChild(this.gfx);this.radius=35;CBullet.list||(CBullet.list=[]);CBullet.list.push(this);var f=this;TweenMax.delayedCall(.05,function(){f.addSmoke()})}
...
CGrenade.makeBoom=function(a,b,c,d,f){ZSound.Play("grenade");var g=pool.Pop("expl");if(g){var h=new CObj(a,b,null);g.loop=!1;g.gotoAndPlay(0);h.allowTrackSpeed=!0;h.gfx=g;h.updateGraphics();SM.inst.fg.addChild(g);h.rotation=Math.random()-.5;g.animationSpeed=.85;h.updateGraphics();h.return2Pool=h.gfx;h.lifeTime=800;h.gfx.onComplete=function(){h.destroy()}}g=d*d;CMonster.list||(l=0);for(var l=CMonster.list.length,k=0;k<l;++k){var m=CMonster.list[k];if(!m.doRemove&&m!=f){var n=m.x-a,p=m.y-b,n=n*n+p*
p;n<g&&(n=Math.sqrt(n),m.dealDamage((d-(n+.01))/d*c));m.doRemove&&(k--,l--)}}this.owner!=gameStage.player&&(n=gameStage.player.x-a,p=gameStage.player.y-b,n*n+p*p<.25*d*d&&gameStage.player.dealDamage(1))};CGrenade.prototype.collide=function(a){this.owner!=gameStage.player&&a&&CObj.checkType(a,CMonster)||a==this.owner||(CGrenade.makeBoom(this.x,this.y,this.dmg,350,this.owner),this.destroy())};CGrenade.prototype.destroy=function(){CBullet.prototype.destroy.call(this)};
...
function CBoosterBox(a,b,c){CObj.apply(this,[a,b,null,null]);this.gravityEnabled=!0;this.colGroup=1;this.colMask=CG_PLAYER;this.gravPower=.46;this.radius=15;this.gfx=crsp("booster box");LauncherBG.inst.ol.addChild(this.gfx);this.updateGraphics();this.allowTrackSpeed=!0}
...
PlayerData.inst.items[a].name!=b.name;++a);var d=PlayerData.inst.getUpgrade(PlayerData.inst.items[a]).upgr,f=new b.cls(SCR_WIDTH/2,SCR_HEIGHT/2,null,d);f.name=b.name;f.item_id=PlayerData.inst.items[a].id;c?(a=c.x,c.duration+=f.duration,f.destroy()):(c&&(b=CBooster.list.indexOf(f),0<=b&&CBooster.list.splice(b,1)),f.gfx=crsp(PlayerData.inst.items[a].gfx),f.gfx.scale.x=.3,f.gfx.scale.y=.3,f.updateGraphics(),new TweenMax(f.gfx.scale,.3,{x:.6,y:.6,repeat:2,yoyo:!0}),a=35+50*(CBooster.list.length-1),new TweenMax(f,
.7,{delay:.8,x:a,y:100}),a=function(){f.onActivate()},c&&(a=function(){f.destroy()}),new TweenMax(f.gfx.scale,.7,{delay:.8,x:.3,y:.3,onComplete:a}),SM.inst.guiLayer.addChild(f.gfx))}};
CBoosterBox.prototype.collide=function(a){if(!this.isCollected)if(this.isCollected=!0,this.gravityEnabled=!1,this.vy=this.vx=0,this.allowTrackSpeed=!1,a=pool.Pop("coinCollect"),ZSound.Play("coin"),a){this.getBooster();rp(a);rp(this.gfx);a.loop=!1;a.gotoAndStop(0);a.gotoAndPlay(0);this.gfx=a;this.updateGraphics();SM.inst.fg.addChild(a);this.updateGraphics();var b=this;b.return2Pool=b.gfx;b.lifeTime=800;b.gfx.onComplete=function(){b.destroy()}}else this.destroy()};extend(CMonster,CLiveObj,!0);
...
MM.prototype.generateMonsterQueue=function(){this.debugArray=[];for(var a=[],b=0,c=0;c<this.patterns.length;++c)this.patterns[c].diff>b&&(b=this.patterns[c].diff);for(c=1;c<=b;++c)a.push({diff:c,prob0:0,prob1:0});for(var b=[],d=0,f=PlayerData.inst.playerItem.lvl/4.5-.5,g,c=0;3E3>c;++c){var h=d/200;h>a.length&&(h=a.length);for(var l=Math.sin(d/50)*(1+.05*h)+h+f,k=h=0;k<a.length;++k){var m=1/(Math.abs(a[k].diff-l)+1),m=Math.pow(m,1.7);0<k?(a[k].prob1=a[k-1].prob1+m,a[k].prob0=a[k-1].prob1):(a[k].prob1=
m,a[k].prob0=0);h+=m}m=Math.random()*h;for(k=0;k<a.length&&!(m>a[k].prob0&&m<=a[k].prob1);++k);for(var h=0,n=a[k].diff,l=a[k].prob1-a[k].prob0,k=0;k<this.patterns.length;++k)this.patterns[k].diff==n&&(m=this.patterns[k].prob,g?(this.patterns[k].prob1=g.prob1+m,this.patterns[k].prob0=g.prob1):(this.patterns[k].prob1=m,this.patterns[k].prob0=0),g=this.patterns[k],h+=m);g=null;m=Math.random()*h;for(k=0;k<this.patterns.length&&!(this.patterns[k].diff==n&&m>this.patterns[k].prob0&&m<=this.patterns[k].prob1);++k);
b+=this.patterns[k].mons;this.debugArray.push({pid:k,diff:this.patterns[k].diff,start:d,prob:(this.patterns[k].prob1-this.patterns[k].prob0)/h,probDiff:l});d+=this.patterns[k].mons.length}this.currentPattern=this.debugArray[0];return b};
...
function CBoosterDrone(a,b,c,d){this.bdy=67;this.bdx=2;this.boosterBox=new CBoosterBox(this.x,this.y+this.bdy);this.boosterBox.allowTrackSpeed=!1;this.boosterBox.gravityEnabled=!1;CMonster.apply(this,[a,b,c,!1]);this.metall=!0}CBoosterDrone.prototype.collide=function(a){};
CBoosterDrone.prototype.dealDamage=function(){this.spawned||(this.boosterBox.gravityEnabled=!0,this.boosterBox.allowTrackSpeed=!0,this.boosterBox.vx=6.5,this.boosterBox.vy=-10,this.boosterBox=null,this.spawned=!0,TweenMax.killTweensOf(this),this.vy=-3,this.vx*=4.4);CLiveObj.prototype.dealDamage.call(this)};CBoosterDrone.prototype.process=function(){this.boosterBox&&(this.boosterBox.x=this.x+this.bdx,this.boosterBox.y=this.y+this.bdy);CMonster.prototype.process.call(this)};extend(CCoin,CObj,!0);
...
function CScrollbar(a,b,c,d,f,g,h,l,k){c="shop back.png";g&&(c=g);CObj.apply(this,[a,b,null,null]);this.gfx=new PIXI.DisplayObjectContainer;this.gui=!0;this.pw=d;this.ph0=this.ph=f;k||(k=0);SM.inst.fg.addChildAt(this.gfx,0);d="scroll line element.png";h&&(d=h);h="scroll.png";l&&(h=l);var m=this;this.gfx.interactive=!0;this.gfx.mouseover=function(){m.mover=!0};this.gfx.mouseout=function(){m.mover=!1};this.bg=new PIXI.Sprite(PIXI.Texture.fromFrame(c));this.bg.width=this.pw+k;this.bg.height=this.ph+
k;this.bg.anchor.x=.5;this.bg.anchor.y=.5;this.gfx.addChild(this.bg);this.updateGraphics();this.toucher=new PIXI.Sprite(PIXI.Texture.fromFrame(h));this.toucher.width=8;this.toucher.anchor.x=.5;this.toucher.anchor.y=.5;this.toucher.x=this.pw/2-4;this.toucher.interactive=!0;this.toucher.scrollbar=this;this.toucher.mousedown=function(a){this.pressed=!0};this.toucher.mouseup=function(a){this.pressed=!1};this.toucher.mouseupoutside=this.toucher.mouseup;this.toucher.mousemove=function(a){this.pressed&&
...
CircleBar.generateProperty("pos",{defaultValue:1,get:function(){return this._pos},set:function(a){0>a&&(a=0);1<a&&(a=1);this._pos=a;this.mask.clear();var b=this.radius;a*=2*Math.PI;var c=Math.cos(a-Math.PI/2)*b,d=Math.sin(a-Math.PI/2)*b,f=[0,-b,0,0];a<Math.PI/2?f.push(c,d):(a<Math.PI?f.push(c,d):(a<3*Math.PI/2?f.push(c,d):(f.push(c,d),f.push(-b,0)),f.push(0,b)),f.push(b,0));this.mask.beginFill(4294967295);this.mask.drawPolygon(f);this.mask.endFill()}});
...
CEActionGUI.prototype.init=function(a,b,c,d,f){CircleBar.prototype.init.call(this,c,d,f);this.eventpl=a;this.event=b;c=0;d="";0<this.event.xp_gain&&(c=this.event.xp_gain,d="price xp.png");0<this.event.money_gain&&(c=this.event.money_gain,d="price coin.png");0<this.event.crystal_gain&&(c=this.event.crystal_gain,d="price star.png");d=new PIXI.Sprite(PIXI.Texture.fromFrame(d));d.anchor.x=.5;d.anchor.y=.5;d.x=170;d.y=-2;this.gfx.addChild(d);this.rewText=CTextField.createTextField({tint:"0x333333",text:"\u041d\u0430\u0433\u0440\u0430\u0434\u0430",
fontSize:18,align:"center"});this.rewText.x=50;this.rewText.y=-12;this.gfx.addChild(this.rewText);this.timeleft=CTextField.createTextField({tint:"0x333333",text:"",fontSize:17,align:"center"});this.timeleft.x=50;this.timeleft.y=27;this.gfx.addChild(this.timeleft);c=CTextField.createTextField({tint:"0x333333",text:c.toString(),fontSize:16,align:"center"});c.x=170;c.y=-11;this.gfx.addChild(c);var g=this,h=this.ico;h.interactive=!0;var l=h.scale.x,k=h.scale.y;h.mouseover=function(){new TweenMax(h.scale,
.6,{y:k+.05,ease:Elastic.easeOut});new TweenMax(h.scale,.4,{x:l+.05,ease:Elastic.easeOut})};h.mouseout=function(){new TweenMax(h.scale,.6,{y:k,ease:Elastic.easeOut});new TweenMax(h.scale,.4,{x:l,ease:Elastic.easeOut})};h.click=function(){g.ready&&!a.reward_ready&&g.startAction()};this.progressbg.interactive=!0;this.progressbg.click=this.onclick;this.progressbg.tap=this.onclick;null==this.eventpl.lastused&&(this.pos=1);c=CTextField.createTextField({fontFamily:"dedgamedesc",tint:"0x1111111",text:b.desc,
...
CWeapon=function(a,b,c,d,f,g){this.lastShot=0;this.backupStats=d;this.resetParams();this.clipGfx=f;this.sReload=1;this.sFire=2;this.visualWidth=d.visualWidth;this.dw=d.dw;this.speed=d.speed;this.state=this.sFire;this.id=a;this.name=b;this.desc=c;this.unlocked=!1;this.upgrades=g;this.sound=d.sound;this.recoilValue=0;this.acc=this.backupStats.acc;CWeapon.list||(CWeapon.objects=[]);CWeapon.objects.push(this)};CWeapon.prototype.mouseUp=function(){};
...
CWeapon.prototype.retrieveData=function(){};CWeapon.prototype.applyUpgrades=function(a){for(var b=0;b<a.length;++b){var c=a[b];this[c.param]+=c.value}};extend(CPistol,CWeapon,!0);function CPistol(a,b,c,d,f,g){CWeapon.apply(this,[a,b,c,d,f,g])}CPistol.prototype.mouseUp=function(){this.lastShot-=.3*this.delay;console.log("MUP")};
CPistol.prototype.shot=function(){var a=CWeapon.prototype.shot.call(this);if(a){var b=gameStage.player.fireAngle+(superRand(6)-.5)*this.acc,c=Math.cos(b),d=Math.sin(b),f=gameStage.player.firePointX,g=gameStage.player.firePointY-14,h=crsp("fxblink.png");h.scale.x=1.3;h.scale.y=1.3;h.x=f+65*c;h.y=g+65*d;h.rotation=b;h.blendMode=PIXI.blendModes.ADD;SM.inst.fg.addChild(h);TweenMax.delayedCall(.03,function(){h.parent.removeChild(h)});50>Math.abs(gameStage.player.y-gameStage.player.baseY)&&(gameStage.player.vx=
-.5);f=new CBullet(f,g,null,null,this.visualWidth);f.life=this.life;f.dmg=this.damage;f.rotation=Math.PI/2+b;f.dw=this.dw;f.vx=c*this.speed;f.vy=d*this.speed;f.colGroup=CG_BULLET;f.colMask=CG_MONSTER;return a}};extend(CQueueGun,CWeapon,!0);function CQueueGun(a,b,c,d,f,g){CWeapon.apply(this,[a,b,c,d,f,g]);this.queueLife=0;this._sound=this.sound;this.sound=null;this.canFireQueue=!0}
...
CQueueGun.prototype.shot=function(a){var b=CWeapon.prototype.shot.call(this);if(b){if(!a){this.canFireQueue=!0;if(0<this.queueLife)return;0!=this.queueLife||a||(this.queueLife=this.queue)}0<this.ammo&&this._sound&&this.queueLife==this.queue&&ZSound.Play(this._sound);this.queueLife--;a=-this.recoilValue/200+gameStage.player.fireAngle+Math.random()*this.backupStats.acc;var c=Math.cos(a),d=Math.sin(a),f=gameStage.player.firePointX,g=gameStage.player.firePointY-14,h=crsp("fxblink.png");h.scale.x=1.3;
h.scale.y=1.3;h.x=f+65*c;h.y=g+65*d;h.rotation=a;h.blendMode=PIXI.blendModes.ADD;SM.inst.fg.addChild(h);TweenMax.delayedCall(.03,function(){h.parent.removeChild(h)});f=new CBullet(f,g,null,null,this.visualWidth);f.life=this.life;f.dmg=this.damage;f.rotation=Math.PI/2+a;f.dw=this.dw;f.vx=c*this.speed;f.vy=d*this.speed;f.colGroup=CG_BULLET;f.colMask=CG_MONSTER;return b}};extend(CLaser,CWeapon,!0);
function CLaser(a,b,c,d,f,g){CWeapon.apply(this,[a,b,c,d,f,g]);this.laserClip=new PIXI.DisplayObjectContainer;this.lineClip=new PIXI.DisplayObjectContainer;this.particlesClip=new PIXI.DisplayObjectContainer;this.laserClip.addChild(this.particlesClip);this.laserClip.addChild(this.lineClip);SM.inst.fg.addChild(this.laserClip);this.updateLaserLen(0);this.lastPart=this.maxL=0}
...
CLaser.prototype.process=function(){CWeapon.prototype.process.call(this);var a=(new Date).getTime();a-this.lastShot>this.delay&&(this.firing=!1);if(this.firing){var b=gameStage.player.fireAngle,c=Math.cos(b),d=Math.sin(b),f=gameStage.player.firePointX,g=gameStage.player.firePointY-14;this.updateLaserLen(this.maxL);this.laserClip.rotation=b-Math.PI/2;this.laserClip.x=f;this.laserClip.y=g;120<a-this.lastPart&&(this.lastPart=a,b=crsp("laserparticle"),b.y=0,b.x=0,b.blendMode=PIXI.blendModes.ADD,this.particlesClip.addChild(b));
for(var h=this.lineClip.children.length,b=0;b<h;++b)this.lineClip.children[b].scale.x=1+b/h,this.lineClip.children[b].x=2.5*Math.cos(b/h*Math.PI*4+a/100);for(b=0;b<this.particlesClip.children.length;++b)h=1-Math.abs(this.maxL/2-this.particlesClip.children[b].y)/(this.maxL/2),.3>h&&(h=.3),this.particlesClip.children[b].scale.x=h,this.particlesClip.children[b].scale.y=-h,h=10+4*Math.cos(a/800),this.particlesClip.children[b].y+=h,this.particlesClip.children[b].y>this.maxL&&(this.particlesClip.removeChildAt(b),
b--);a=g+1E4*d;c=f+1E4*c;if(CMonster.list)for(b=0;b<CMonster.list.length;++b){var l=CMonster.list[b].x,d=CMonster.list[b].y,h=((a-g)*(l-f)-(c-f)*(d-g))/((a-g)*(a-g)+(c-f)*(c-f)),l=l-(l-h*(a-g)),d=d-(d+h*(c-f));l*l+d*d<CMonster.list[b]._sqr&&CMonster.list[b].dealDamage(this.damage/60)}this.laserClip.visible=!0}else this.laserClip.visible=!1};CLaser.prototype.shot=function(){CWeapon.prototype.shot.call(this)&&!this.firing&&(this.maxL=0,new TweenMax(this,.6,{maxL:1E3}),this.firing=!0)};
extend(CGrenadeLauncher,CWeapon,!0);function CGrenadeLauncher(a,b,c,d,f,g){CWeapon.apply(this,[a,b,c,d,f,g])}
CGrenadeLauncher.prototype.shot=function(){var a=CWeapon.prototype.shot.call(this);if(a){var b=gameStage.player.fireAngle+Math.random()*this.backupStats.acc,c=Math.cos(b),d=Math.sin(b),f=gameStage.player.firePointX,g=gameStage.player.firePointY-14,h=crsp("fxblink.png");h.x=f+50*c;h.y=g+50*d;h.rotation=b;h.blendMode=PIXI.blendModes.ADD;SM.inst.fg.addChild(h);TweenMax.delayedCall(.03,function(){h.parent.removeChild(h)});f=new CGrenade(f,g);f.owner=gameStage.player;f.life=this.life;f.dmg=this.damage;
f.rotation=Math.PI/2+b;f.av=.2;f.vx=17.5*c;f.vy=17.5*d;f.gravityEnabled=!0;f.colGroup=CG_BULLET;f.colMask=CG_MONSTER;return a}};
...
CBooster.prototype.process=function(){if(this.activate&&!this.doRemove){if(this.tf){if(1E3<PauseTimer.getTimer()-this.lastTick){this.lastTick=PauseTimer.getTimer();var a=Math.round(this.duration-(PauseTimer.getTimer()-this.startTime)/1E3);0>a&&(a=0);this.tf.text=a.toString();this.tf.updateText()}this.tf.x=-this.tf.width/2}if(this.startTime&&PauseTimer.getTimer()-this.startTime>1E3*this.duration)this.onDeactivate()}};extend(CDoubleBooster,CBooster,!0);
...
CTabletsBooster.prototype.onDeactivate=function(){new TweenMax(LauncherBG.inst,.3,{maxVelocity:this.prevVel,ease:Linear.easeOut});CBooster.prototype.onDeactivate.call(this);var a=gameStage.player;a.superMode=!1;a.resetBlink();TweenMax.delayedCall(.7,function(){a.invulnerable--});a.blink=!1};
...
PlayerData.prototype.getUpgrade=function(a,b){if(b)for(var c=0;c<this.items.length;++c)if(this.items[c].name==b){a=this.items[c];break}for(var d=a.name,f=[],c=0;c<this.items.length;++c)0<=this.items[c].name.indexOf(d+"$")&&f.push({id:this.items[c].id,name:this.items[c].name});f.sort(function(a,b){return b.name.localeCompare(a.name)});d=!1;for(c=0;c<f.length;++c)for(var g=0;g<PlayerData.inst.items_enabled.length;++g)if(PlayerData.inst.items_enabled[g].id_item==a.id&&(d=!0),PlayerData.inst.items_enabled[g].id_item==
f[c].id)return{upgr:5-c,id:f[c].id,idNext:0<c?f[c-1].id:null};return d?{upgr:1,id:a.id,idNext:f[f.length-1].id}:{upgr:0,id:null,idNext:a.id}};PlayerData.prototype.equipItem=function(a){for(var b=a.id,c=!1,d=0;d<this.items_enabled.length;++d)this.items_enabled[d].id_item==b&&(c=!0);if(c){a=a.type;for(d=0;d<PlayerData.inst.items_enabled.length;++d)this.getType(PlayerData.inst.items_enabled[d])==a&&(PlayerData.inst.items_enabled[d].equipped=PlayerData.inst.items_enabled[d].id_item==b?!0:!1);this.savePlayerItems()}};
...
b&&(gameStage.updateScore(),gameStage.pause()),LevelManager.loadLevel("levelup",function(){CObj.getById("moneytf").text=PlayerData.inst.xpLevel[PlayerData.inst.playerItem.lvl].money.toString();CObj.getById("crystalstf").text=PlayerData.inst.xpLevel[PlayerData.inst.playerItem.lvl].crystals.toString();CObj.getById("levnum").text=PlayerData.inst.playerItem.lvl.toString();CObj.getById("btnclose").click=function(){LevelManager.removeLastLevel();b?gameStage.unpause():(shopStage.updateStatsPanel(),charStage.closeEventsWnd(),
PlayerData.inst.savePlayerData())}},SM.inst.fontLayer));SM.inst.currentStage==gameStage&&gameStage.updateXP()};
...
PlayerData.prototype.progressAch=function(a,b,c){for(var d=0;d<this.achs.length&&this.achs[d].name.toLowerCase()!=a.toLowerCase();++d);a=!1;for(var f=0;f<this.achs_progress.length;++f)if(this.achs_progress[f].id_ach==this.achs[d].id){c?(1<=b&&(a=!0),this.achs_progress[f].progress=b):(1>this.achs_progress[f].progress&&1<=this.achs_progress[f].progress+b&&(a=!0,charStage.unreadAch=!0),this.achs_progress[f].progress+=b);break}a&&(this.savePlayerAchs(this.achs_progress[f]),this.showAch(this.achs[d]),
incMetric("ACH GAINED "+this.achs[d].name));return a};PlayerData.prototype.loadEnd=function(){PlayerData.inst.createAchProgress();window.dbinit=!0;onAssetsLoaded()};
...
b;PlayerData.inst.items.sort(function(a,b){return a.reqlvl-b.reqlvl});PlayerData.inst.loadCount++;7==PlayerData.inst.loadCount&&a&&a()},function(a){});window.azureclient.getTable("tb_item_player").read().done(function(b){PlayerData.inst.items_enabled=b;for(var c=b=!1,d=-1,f=0;f<PlayerData.inst.items_enabled.length;++f)"68AFAEDC-B3E0-401E-9E1A-E272084F2E11"==PlayerData.inst.items_enabled[f].id_item&&(d=f,b=!0),PlayerData.inst.items_enabled[f].equipped&&(c=!0);!c&&b&&(PlayerData.inst.items_enabled[d].equipped=
...
gfx:"orden6.png"},{name:"Gold medal 11",desc:"\u041d\u0430\u043a\u0430\u0437\u0430\u0442\u044c 200 \u0448\u043a\u043e\u043b\u044c\u043d\u0438\u043a\u043e\u0432",gfx:"orden12.png"}]},{tbname:"tb_items",items:[{type:tWeapon,price:-1,pricecrys:-1,name:"Rifle",desc:"\u0421\u0442\u0430\u0440\u043e\u0435 \u0434\u0435\u0434\u043e\u0432\u0441\u043a\u043e\u0435 \u0440\u0443\u0436\u044c\u0438\u0448\u043a\u043e",gfx:"gun0",reqlvl:-1},{type:tWeapon,price:600,pricecrys:12,name:"PPS",desc:"\u041f\u041f\u0428|\u041f\u0438\u0441\u0442\u043e\u043b\u0435\u0442-\u043f\u0443\u043b\u0435\u043c\u0451\u0442 \u0428\u043f\u0430\u0433\u0438\u043d\u0430==\u043c\u0430\u0433\u0430\u0437\u0438\u043d 35 \u043f\u0430\u0442\u0440\u043e\u043d\u043e\u0432",
gfx:"gun1",reqlvl:2},{type:tWeapon,price:1300,pricecrys:26,name:"AK-74",desc:"\u041a\u0430\u043b\u0430\u0448|\u0410\u041a-47 - \u0430\u0432\u0442\u043e\u043c\u0430\u0442 \u0431\u044b\u043b \u0441\u043a\u043e\u043d\u0441\u0442\u0440\u0443\u0438\u0440\u043e\u0432\u0430\u043d==\u0432 1947 \u0433\u043e\u0434\u0443 \u041c\u0438\u0445\u0430\u0438\u043b\u043e\u043c \u0422\u0438\u043c\u043e\u0444\u0435\u0435\u0432\u0438\u0447\u0435\u043c \u041a\u0430\u043b\u0430\u0448\u043d\u0438\u043a\u043e\u0432\u044b\u043c==\u0423\u0432\u0435\u043b\u0438\u0447\u0435\u043d\u043d\u044b\u0439 \u043c\u0430\u0433\u0430\u0437\u0438\u043d: 50 \u043f\u0430\u0442\u0440\u043e\u043d\u043e\u0432",
...
vkparams.friendsIngame=c.result;vkparams.friendsIngameIDs=[];if(c.result){for(var f=0;f<c.result.length;++f)vkparams.friendsIngameIDs.push(c.result[f].vkapi);2==b&&new PlayerData(a)}},function(a){})});VK.api("users.get",{user_ids:vkparams.viewerid.toString()},function(c){vkparams.first_name=c.response[0].first_name?c.response[0].first_name:"\u041d\u043e\u0443\u043d\u0435\u0439\u043c";vkparams.last_name=c.response[0].first_name?c.response[0].last_name:"";b++;2==b&&new PlayerData(a)})}};
...
getDedImage=function(a){shopStage.transScreen&&(shopStage.transScreen.visible=!1);if(a)var b=300,c=365,d=-55,f=-120;else b=SCR_WIDTH/2,c=SCR_HEIGHT,f=d=0;b=new PIXI.RenderTexture(b,c);a&&(CObj.getById("bback").gfx.visible=!1,CObj.getById("ava").gfx.visible=!1,CObj.getById("bgshopded").gfx.visible=!1);renderer.render(stage,!0);c=new PIXI.Matrix;c.translate(d,f);b.render(SM.inst.superStage,c);d=b.getBase64();b.destroy(!0);a&&(CObj.getById("bgshopded").gfx.visible=!0,CObj.getById("bback").gfx.visible=
...
function post(a,b,c){c=c||"post";var d=document.createElement("form");d.setAttribute("method",c);d.setAttribute("action",a);for(var f in b)b.hasOwnProperty(f)&&(a=document.createElement("input"),a.setAttribute("type","hidden"),a.setAttribute("name",f),a.setAttribute("value",b[f]),d.appendChild(a));document.body.appendChild(d);d.submit()}PauseTimer=function(){};PauseTimer.paused=!1;PauseTimer.pauseStartedAt=0;PauseTimer.totalPauseTime=0;PauseTimer.getTimer=function(){return window.time-PauseTimer.totalPauseTime};
PauseTimer.isPaused=function(){return PauseTimer.paused};PauseTimer.pause=function(){PauseTimer.pauseStartedAt=PauseTimer.getTimer();TweenMax.pauseAll();PauseTimer.paused=!0};PauseTimer.resume=function(){PauseTimer.paused&&(PauseTimer.totalPauseTime+=window.time-PauseTimer.pauseStartedAt,TweenMax.resumeAll(),PauseTimer.paused=!1)};window.SCR_WIDTH=800;window.SCR_HEIGHT=600;window.SCR_SCALE=1;window.FRAME_RATE=60;var loadingState="prepreload";window.openSponsorWindow=null;window.focus();
var assetsLoaded=0,preloaderAsset=["imgtps/preloader.json"];window.addScale=1;window.renderer=new PIXI.autoDetectRenderer(window.SCR_WIDTH,window.SCR_HEIGHT);window.loader=new PIXI.AssetLoader(preloaderAsset);window.loader.onComplete=preloaderLoaded;window.loader.load();dbInit();$(document).bind("contextmenu",function(){return!1});window.apiid=4654201;VK.init({apiId:window.apiid},function(){},function(){});
...
function showADs(){var a=document.createElement("div");a.id="vk_ads_55316";a.setAttribute("style","position:absolute;left:0%;top:0%;");document.body.appendChild(a);setTimeout(function(){function a(){VK.Widgets.Ads("vk_ads_55316",{},c)}var c={ad_unit_id:55316,ad_unit_hash:"d4685898a5210c69b772bf2ab6fb0571"};if(window.VK&&VK.Widgets)a();else{window.vkAsyncInitCallbacks||(window.vkAsyncInitCallbacks=[]);vkAsyncInitCallbacks.push(a);var d="https:"===location.protocol?"https:":"http:",f=document.getElementById("vk_ads_55316"),
g=document.createElement("script");g.type="text/javascript";g.async=!0;g.src=d+"//vk.com/js/api/openapi.js?116";f.parentNode.insertBefore(g,f.nextSibling)}},0)}
function preloaderLoaded(){window.stage=new PIXI.Stage(16777215);window.px=SCR_WIDTH;window.py=SCR_HEIGHT;window.loadingScreen=new PIXI.Graphics;window.preloaderBg=new PIXI.DisplayObjectContainer;var a=crsp("1st plan");a.x=SCR_WIDTH/2;a.y=SCR_HEIGHT/2;a.scale.x=1.1;a.scale.y=1.1;var b=crsp("background");b.x=SCR_WIDTH/2;b.y=SCR_HEIGHT/2;b.scale.x=1.1;b.scale.y=1.1;preloaderBg.addChild(b);preloaderBg.addChild(a);a=crsp("loading bar");a.x=SCR_WIDTH/2;a.y=SCR_HEIGHT/2+160;preloaderBg.addChild(a);SM.inst.addLayersToStage();
