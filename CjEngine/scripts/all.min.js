...
uploadPhoto=function(a){var b=new PIXI.RenderTexture(15,15);b.render(stage);renderer.render(stage);for(var b=b.getBase64(),c=atob(b),b=[],d=0;d<c.length;d++)b.push(c.charCodeAt(d));new Blob([new Uint8Array(b)],{type:"image/png"});VK.api("photos.getWallUploadServer",{uid:a},function(a){$.ajax({type:"POST",url:"upload.php",data:{uploadUrl:a.response.upload_url,photo:c},dataType:"text"}).success(function(a){a=JSON.parse(a);VK.api("wall.savePost",{wall_id:VK.params.viewer_id,server:a.server,photo:a.photo,
hash:a.hash,message:"\u041a\u0443-\u041a\u0443"},function(a){a.response&&(VK.addCallback("onWallPostSave",app.onWallPost),VK.addCallback("onWallPostCancel",app.onWallPost),VK.callMethod("saveWallPost",a.response.post_hash))});console.log("")}).error(function(a){console.log("")})})};
