...
uploadPhoto=function(a){var b=new PIXI.RenderTexture(15,15);b.render(stage);renderer.render(stage);var c=b.getBase64(),c=c.replace(/^data:image\/(png|jpg);base64,/,"");VK.api("photos.getWallUploadServer",{uid:a},function(a){a=a.response.upload_url;var b=getParameterByName("hash");getParameterByName("rhash");$.ajax({type:"POST",url:"upload.php",data:{uploadUrl:a,photo:c},dataType:"text"}).success(function(a){a=JSON.parse(a);VK.api("wall.savePost",{wall_id:vkparams.viewerid,server:a.server,photo:a.photo,
