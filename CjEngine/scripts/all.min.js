...
uploadPhoto=function(a){var b=new PIXI.RenderTexture(SCR_WIDTH/2,SCR_HEIGHT);renderer.render(stage,!0);b.render(SM.inst.superStage);var c=b.getBase64();b.destroy(!0);c=c.replace(/^data:image\/(png|jpg);base64,/,"");VK.api("photos.getWallUploadServer",{uid:a},function(a){a=a.response.upload_url;getParameterByName("hash",a);getParameterByName("rhash",a);$.ajax({type:"POST",url:"upload.php",data:{uploadUrl:a,photo:c},dataType:"text"}).success(function(a){a=JSON.parse(a);VK.api("photos.saveWallPhoto",
