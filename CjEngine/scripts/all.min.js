...
rgbtoint=function(a,b,c){return 16777216+(a<<16)+(b<<8)+c};function rgbToHsl(a,b,c){a/=255;b/=255;c/=255;var d=Math.max(a,b,c),e=Math.min(a,b,c),f,g=(d+e)/2;if(d==e)f=e=0;else{var h=d-e,e=.5<g?h/(2-d-e):h/(d+e);switch(d){case a:f=(b-c)/h+(b<c?6:0);break;case b:f=(c-a)/h+2;break;case c:f=(a-b)/h+4}f/=6}return[f,e,g]}
...
if(!vkparams.friendsIngame||e>=vkparams.friendsIngame.length)g.click=function(){VK.callMethod("showInviteBox")};else{var f=function(a,b,c){c.click=function(){VK.api("wall.post",{owner_id:b,message:a+", \u0432\u043e\u0437\u0432\u0440\u0430\u0449\u0430\u0439\u0441\u044f \u0432 \u0438\u0433\u0440\u0443, \u0434\u0440\u0443\u0436\u0438\u0449\u0435\nhttps://vk.com/app4654201",attachments:["photo90523698_359515858","https://vk.com/app4654201"]},function(a){})}},h=vkparams.friendsIngame[e].name,k=vkparams.friendsIngame[e].vkapi;
h&&k&&f(h,k,g);vkparams.friendsIngame[e].vkapi&&(""!=b&&(b+=","),b+=vkparams.friendsIngame[e].vkapi.toString(),g.vkapi=vkparams.friendsIngame[e].vkapi);c.push(g)}}VK&&""!=b&&VK.api("users.get",{user_ids:b,fields:"photo"},function(a){if(a.response)for(var b=0;b<a.response.length;++b){var d=a.response[b].photo;if(!a.response||0==a.response.length)break;var e=c[b];e.loader=new PIXI.ImageLoader(d);(function(a,b){a.loader.onLoaded=function(){var c=new PIXI.Sprite(PIXI.TextureCache[b]);c.anchor.x=.5;c.anchor.y=
...
CharStage.prototype.updateSB=function(a){for(;0<charStage.container.children.length;)rp(charStage.container.getChildAt(0));for(var b=[],c="",d=0;d<a.length;++d){var e=new PIXI.DisplayObjectContainer,f=CTextField.createTextField({tint:3355443,fontSize:17,text:(charStage.skip+d+1).toString()+"."}),g=CTextField.createTextField({tint:3355443,fontSize:17,text:extractName(a[d])}),h=CTextField.createTextField({tint:3355443,fontSize:17,text:Math.round(a[d].maxdistance).toString()+"\u043c."}),k=crsp("ava cover");
b.push(k);b[d].vkapi=a[d].vkapi;""!=c&&(c+=",");c+=a[d].vkapi;if(3>charStage.skip+d){var l;0==charStage.skip+d&&(l="1st place");1==charStage.skip+d&&(l="2nd place");2==charStage.skip+d&&(l="3rd place");var m=crsp(l);m.x=4;m.y=10;m.scale.x=.8;m.scale.y=.8;e.addChild(m)}k.y=10;k.x=35;e.x=20;e.y=15+28*d;g.x=55;h.x=370;e.addChild(k);e.addChild(g);e.addChild(f);e.addChild(h);charStage.container.addChild(e)}VK&&""!=c&&VK.api("users.get",{user_ids:c,fields:"photo"},function(a){if(a.response)for(var c=0;c<
...
c.setTextSafe(b);c.item=a};l.mouseout=function(b){b=CObj.getById("tfdescd");b.item==a&&b.setTextSafe("");a.type!=tWeapon&&a.type!=tApp+tHat||shopStage.pl.updateAppearence(!0,!1,null,null,null);l.tint=16777215;l.currentFrame&&l.gotoAndStop(1);new TweenMax(l.scale,.3,{x:.5,y:.5,ease:Elastic.easeOut})};if(f)for(var m=0;5>m;++m){var n=m<h?crsp("improve point on"):crsp("improve point off");n.x=25*(m-2);n.scale.x=.8;n.scale.y=.8;n.y=3;e.addChild(n)}m=this.checkOwned(a);n=this.checkEq(a);tftext="";f&&(k?
tftext="":m||(tftext=0>a.pricecrys&&0>a.price?"":0<a.pricecrys?a.pricecrys.toString():a.price.toString()));var q,k="",p="buy button",r;!m||f?f&&5==h?p="equipped button":a.reqlvl>PlayerData.inst.playerItem.lvl?(q=b,p="unlock button",k="\u041c\u0418\u041d "+a.reqlvl.toString()+" \u0423\u0420.",r="price star.png",tftext=a.pricecrys.toString()):(q=d,r="price coin.png",p="buy button",tftext=a.price.toString()):1==n?p="equipped button":(q=c,p="wear button");h=new CButton(0,129,p);"equipped button"==p&&
(h.gfx.mouseout=null,h.gfx.mouseover=null);h.fontSize=33;h.addToSameLayer=!0;h.gfx.anchor.x=.5;h.gfx.anchor.y=.5;h.gfx.scale.x=.8;h.gfx.scale.y=.8;h.init();h.gfx.parent.removeChild(h.gfx);h.item=f?PlayerData.inst.getItemById(g.idNext):a;h.gfx.btn=h;h.click=q;e.addChild(h.gfx);h.text&&(h.text=h.text);e.btn=h;tftext&&r&&(f=crsp(r),f.x=0,f.y=80,f.scale.x=.75,f.scale.y=.75,e.addChild(f));f=new CTextField;f.tint="0x333333";f.fontSize=15;f.align="center";f.init();e.infoTF=f;f.x=0;f.y=158;f.gfx.parent.removeChild(f.gfx);
f.text=k;e.addChild(f.gfx);f=new CTextField;f.tint="0x333333";f.fontSize=15;f.align="center";f.text=tftext;f.init();e.tfprice=f;f.x=10;f.y=70;f.gfx.parent.removeChild(f.gfx);e.addChild(f.gfx);f.text=f.text;e.destroy=function(){e.tfprice.destroy();e.btn&&(e.btn.destroy(),e.btn=null);e.tfprice=null;e.parent.removeChild(e)};return e};
...
new TweenMax(a.scale,.4,{x:1.1,ease:Elastic.easeOut}));if(e.hover){var g=CObj.getById("descpanel");if(g){var h=e.text;setTimeout(function(){g.text=h;CObj.getById("descbg").gfx.visible=!0},100)}else e.textField.alpha=1}};this.gfx.mouseout=function(f){e.over=!1;b.tint=16777215;b.currentFrame&&b.gotoAndStop(1);new TweenMax(b.scale,.3,{x:c,y:d,ease:Elastic.easeOut});a.visible&&""!=a.text&&new TweenMax(a.scale,.3,{x:1,y:1,ease:Elastic.easeOut});if(e.hover){var g=CObj.getById("descpanel");g?setTimeout(function(){g.text=
" ";CObj.getById("descbg").gfx.visible=!1},0):e.textField.alpha=0}};this.addToSameLayer?this.gfx.addChild(this.textField):SM.inst.fontLayer.addChild(this.textField)};extend(ZCJoint,CObj,!0);function ZCJoint(a,b,c,d){CObj.apply(this,[a,b,c,d]);this.PublicFields+="body1Id,body2Id,collideConnected,breakOnForce,angleConstraint,angleMin,angleMax,frequency,damping,rate,isStopped,"}
ZCJoint.prototype.CreateMotor=function(){joint&&1E-6<Math.abs(rate)&&(motor||(motor=new MotorJoint(joint.body1,joint.body2,2*Math.PI*rate)),motor.rate=isStopped?0:2*Math.PI*rate,motor.ignore=!collideConnected,motor.space=ZNape.Inst.World)};ZCJoint.getFirstStatic=function(){for(var a=CObj.objects.length,b=0;b<a;++b)if(CObj.objects[b].body&&CObj.objects[b].body.type==p2.Body.STATIC)return CObj.objects[b].body};
...
CGrenade.makeBoom=function(a,b,c,d,e){ZSound.Play("grenade");var f=pool.Pop("expl");if(f){var g=new CObj(a,b,null);f.loop=!1;f.gotoAndPlay(0);g.allowTrackSpeed=!0;g.gfx=f;g.updateGraphics();SM.inst.fg.addChild(f);g.rotation=Math.random()-.5;f.animationSpeed=.85;g.updateGraphics();g.return2Pool=g.gfx;g.lifeTime=800;g.gfx.onComplete=function(){g.destroy()}}f=d*d;CMonster.list||(h=0);for(var h=CMonster.list.length,k=0;k<h;++k){var l=CMonster.list[k];if(!l.doRemove&&l!=e){var m=l.x-a,n=l.y-b,m=m*m+n*
n;m<f&&(m=Math.sqrt(m),l.dealDamage((d-(m+.01))/d*c));l.doRemove&&(k--,h--)}}this.owner!=gameStage.player&&(m=gameStage.player.x-a,n=gameStage.player.y-b,m*m+n*n<.25*d*d&&gameStage.player.dealDamage(1))};CGrenade.prototype.collide=function(a){this.owner!=gameStage.player&&a&&CObj.checkType(a,CMonster)||a==this.owner||(CGrenade.makeBoom(this.x,this.y,this.dmg,350,this.owner),this.destroy())};CGrenade.prototype.destroy=function(){CBullet.prototype.destroy.call(this)};
...
MM.prototype.generateMonsterQueue=function(){this.debugArray=[];for(var a=[],b=0,c=0;c<this.patterns.length;++c)this.patterns[c].diff>b&&(b=this.patterns[c].diff);for(c=1;c<=b;++c)a.push({diff:c,prob0:0,prob1:0});for(var b=[],d=0,e=PlayerData.inst.playerItem.lvl/4.5-.5,f,c=0;3E3>c;++c){var g=d/200;g>a.length&&(g=a.length);for(var h=Math.sin(d/50)*(1+.05*g)+g+e,k=g=0;k<a.length;++k){var l=1/(Math.abs(a[k].diff-h)+1),l=Math.pow(l,1.7);0<k?(a[k].prob1=a[k-1].prob1+l,a[k].prob0=a[k-1].prob1):(a[k].prob1=
l,a[k].prob0=0);g+=l}l=Math.random()*g;for(k=0;k<a.length&&!(l>a[k].prob0&&l<=a[k].prob1);++k);for(var g=0,m=a[k].diff,h=a[k].prob1-a[k].prob0,k=0;k<this.patterns.length;++k)this.patterns[k].diff==m&&(l=this.patterns[k].prob,f?(this.patterns[k].prob1=f.prob1+l,this.patterns[k].prob0=f.prob1):(this.patterns[k].prob1=l,this.patterns[k].prob0=0),f=this.patterns[k],g+=l);f=null;l=Math.random()*g;for(k=0;k<this.patterns.length&&!(this.patterns[k].diff==m&&l>this.patterns[k].prob0&&l<=this.patterns[k].prob1);++k);
b+=this.patterns[k].mons;this.debugArray.push({pid:k,diff:this.patterns[k].diff,start:d,prob:(this.patterns[k].prob1-this.patterns[k].prob0)/g,probDiff:h});d+=this.patterns[k].mons.length}this.currentPattern=this.debugArray[0];return b};
...
fontSize:18,align:"center"});this.rewText.x=50;this.rewText.y=-12;this.gfx.addChild(this.rewText);this.timeleft=CTextField.createTextField({tint:"0x333333",text:"",fontSize:17,align:"center"});this.timeleft.x=50;this.timeleft.y=27;this.gfx.addChild(this.timeleft);c=CTextField.createTextField({tint:"0x333333",text:c.toString(),fontSize:16,align:"center"});c.x=170;c.y=-11;this.gfx.addChild(c);var f=this,g=this.ico;g.interactive=!0;var h=g.scale.x,k=g.scale.y;g.mouseover=function(){new TweenMax(g.scale,
.6,{y:k+.05,ease:Elastic.easeOut});new TweenMax(g.scale,.4,{x:h+.05,ease:Elastic.easeOut})};g.mouseout=function(){new TweenMax(g.scale,.6,{y:k,ease:Elastic.easeOut});new TweenMax(g.scale,.4,{x:h,ease:Elastic.easeOut})};g.click=function(){f.ready&&!a.reward_ready&&f.startAction()};this.progressbg.interactive=!0;this.progressbg.click=this.onclick;this.progressbg.tap=this.onclick;null==this.eventpl.lastused&&(this.pos=1);c=CTextField.createTextField({fontFamily:"dedgamedesc",tint:"0x1111111",text:b.desc,
