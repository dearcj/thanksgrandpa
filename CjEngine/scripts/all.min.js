...
new TweenMax(LauncherBG.inst,2,{maxVelocity:LauncherBG.inst.preVelocity})};if(PlayerData.inst.playerItem.crystals>gameStage.revealPrice)c();else{var d=gameStage.revealPrice-PlayerData.inst.playerItem.crystals;3>=d?order("item4"):15>=d?order("item5"):order("item6");VK.orderComplete=function(){PlayerData.inst.playerItem.crystals>price&&c()};d=function(){b.resume()};VK.orderFail=d;VK.orderCancel=d}});var d=a.gfx.scale.x,e=a.gfx.scale.y;a.gfx.mouseover=function(b){TweenMax.killTweensOf(a.gfx.scale);a.gfx.tint=
CButton.tintColor;new TweenMax(a.gfx.scale,.6,{y:e+.05,ease:Elastic.easeOut});new TweenMax(a.gfx.scale,.4,{x:d+.05,ease:Elastic.easeOut})};a.gfx.mouseout=function(b){a.gfx.tint=16777215;new TweenMax(a.gfx.scale,.3,{x:d,y:e,ease:Elastic.easeOut})};var f=new PIXI.Sprite(PIXI.Texture.fromFrame("bodrost star.png"));f.anchor.x=.5;f.anchor.y=.5;f.x=SCR_WIDTH/2;f.y=SCR_HEIGHT/2+120;SM.inst.guiLayer.addChild(f);var g=CTextField.createTextField({text:gameStage.revealPrice.toString(),fontSize:22,align:"center"});
g.tint=3355443;g.updateText();g.x=8+SCR_WIDTH/2;g.y=SCR_HEIGHT/2+107;SM.inst.guiLayer.addChild(g)}};
...
PlayerData.dbInit=function(){console.log("dbInit start. Connecting to azure");window.azureclient=new WindowsAzure.MobileServiceClient("https://thanksdad.azure-mobile.net/","DRoaNHnoaCjxrhkbpOzHxGEHOFgGLS75");window.vkparams={};vkparams.userid=getURLParameter("user_id");vkparams.sid=getURLParameter("sid");vkparams.viewerid=getURLParameter("viewer_id");vkparams.viewerid&&(vkparams.viewerid=vkparams.viewerid.toString());vkparams.viewerid&&VK||(vkparams.viewerid="CARLAPA2882845",MOBILE&&(vkparams.viewerid=
