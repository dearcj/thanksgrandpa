var CG_GROUND=1,CG_MONSTER=2,CG_PLAYER=4,CG_BULLET=8;function isValidDate(a){return"[object Date]"!==Object.prototype.toString.call(a)?!1:!isNaN(a.getTime())}function datetime(){return window.date?window.date:new Date}function ajaxCalltoGetTime(){PlayerData.inst.callDedAPI("GET_DATE",null,null,null,function(a){isNaN((new Date(a)).getTime())||(window.date=new Date(a))})}function startServerTimeTicker(){ajaxCalltoGetTime();setInterval(ajaxCalltoGetTime,1E4)}
function getCookie2(a){return document.cookie.match(new RegExp("(?:^|; )"+a.replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g,"\\$1")+"=([^;]*)"))[1]}function getCookie(a){a+="=";for(var b=document.cookie.split(";"),c=0;c<b.length;c++){for(var d=b[c];" "==d.charAt(0);)d=d.substring(1);if(0==d.indexOf(a))return d.substring(a.length,d.length)}return""}function strad(a,b){return parseInt(a)+parseInt(b)}function noc(a,b){a.click=null;a.tap=null}function onc(a,b){a.click=b;a.tap=b}
function extractName(a){return a.name?(a.name+" "+a.last_name).substring(0,16):"\u0410\u043d\u043e\u043d\u0438\u043c"}function findByProp(a,b,c){for(var d=a.length,e=0;e<d;++e)if(a[e][b]==c)return a[e];return null}function getRand(a){return a[Math.floor(Math.random()*a.length)]}function OpenInNewTab(a){window.open(a,"_blank").focus()}function clone(a){return JSON.parse(JSON.stringify(a))}
function sqlToJsDate(a){if(!a)return new Date;var b=a.split("-");a=b[0];var c=(Number(b[1])-1).toString(),d=b[2].split(" "),b=d[0],e=d[1].split(":"),d=e[0],f=e[1],e=e[2].split(".")[0];return new Date(a,c,b,d,f,e,0)}function sqlDate(a){return a.getFullYear()+"-"+("00"+(a.getMonth()+1)).slice(-2)+"-"+("00"+a.getDate()).slice(-2)+" "+("00"+a.getHours()).slice(-2)+":"+("00"+a.getMinutes()).slice(-2)+":"+("00"+a.getSeconds()).slice(-2)}
