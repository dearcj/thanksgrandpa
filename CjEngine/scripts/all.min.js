...
console.log("FILTERING AZURE RECORDS WITH "+JSON.stringify(vkparams.friendsids));console.log("TOTAL FILTERS: "+vkparams.friendsids.length.toString());azureclient.invokeApi("get_scores",{body:{filter:vkparams.friendsids},method:"post"}).done(function(b){console.log("AZURE RECORDS: "+JSON.stringify(b.result));vkparams.friendsIngame=b.result;vkparams.friendsIngameIDs=[];if(b.result){for(var d=0;d<b.result.length;++d)vkparams.friendsIngameIDs.push(b.result[d].vkapi);PlayerData.getVKuserData(a)}},function(a){})}else PlayerData.getVKfriends(a)}))};
