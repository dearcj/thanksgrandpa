...
uploadPhoto=function(a){var b=new PIXI.RenderTexture(SCR_WIDTH,SCR_HEIGHT);b.render(SM.inst.stage);renderer.render(stage);b=b.getBase64();window.atob(b);VK.api("photos.getWallUploadServer",{uid:a},function(a){$.ajax({type:"POST",url:"upload.php",data:{uploadUrl:a.response.upload_url},dataType:"text"}).success(function(a){a=JSON.parse(a);VK.api("wall.savePost",{wall_id:VK.params.viewer_id,server:a.server,photo:a.photo,hash:a.hash,message:"\u041a\u0443-\u041a\u0443"},function(a){a.response&&(VK.addCallback("onWallPostSave",
app.onWallPost),VK.addCallback("onWallPostCancel",app.onWallPost),VK.callMethod("saveWallPost",a.response.post_hash))});console.log("")}).error(function(a){console.log("")})})};
