var CG_GROUND=1,CG_MONSTER=2,CG_PLAYER=4,CG_BULLET=8;function ajaxCalltoGetTime(){$.ajax({type:"GET",url:"http://json-time.appspot.com/time.json",dataType:"jsonp"}).done(function(a){window.date=new Date(a.hour+":"+a.minute+":"+a.second)})}function startServerTimeTicker(){setInterval(ajaxCalltoGetTime,1E3)}function getCookie2(a){return document.cookie.match(new RegExp("(?:^|; )"+a.replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g,"\\$1")+"=([^;]*)"))[1]}
...
"\u043e\u043f\u0438\u0441\u0430\u043d\u0438\u0435",{recoilValue:0,visualWidth:20,dw:-.7,speed:40.5,sound:"laser",life:1,acc:.08,accIncrease:0,accRecoil:0,recoil:0,magcap:60,delay:150,damage:190,unlockprice:10,reloadTime:2200},"asd",[{name:"\u0422\u043e\u0447\u043d\u044b\u0439 \u0441\u0442\u0432\u043e\u043b",param:"recoil",valuesUpgr:[10,20,30,40],level:0},{name:"\u0414\u043e\u043f\u043e\u043b\u043d\u0438\u0442\u0435\u043b\u044c\u043d\u044b\u0439 \u043c\u0430\u0433\u0430\u0437\u0438\u043d",param:"magCapacity",
...
PlayerData.prototype.comboCheck=function(){if(this.playerItem.combodate){var a=(new Date).getTime()-(new Date(sqlToJsDate(this.playerItem.updateDate))).getTime(),a=a/1E3/60;1740<a?this.playerItem.combodate=sqlDate(new Date):(a=(new Date).getTime()-(new Date(sqlToJsDate(this.playerItem.combodate))).getTime(),a/=1E3,a/=60,4320<a&&this.progressAch("Gold medal 8",1,!1),14400<a&&this.progressAch("Gold medal 9",1,!1))}else this.playerItem.combodate=this.playerItem.updateDate};
...
PlayerData.prototype.updateEnergy=function(a){if(this.playerItem){var b=sqlToJsDate(this.playerItem.updateDate),b=(new Date).getTime()-b.getTime();this.playerItem.updateDate=sqlDate(new Date);b=b/1E3/60;if(0<b){var c=this.playerItem.energy;this.playerItem.energy+=b*this.epm;console.log(this.playerItem.energy);this.playerItem.energy>this.maxEnergy&&(this.playerItem.energy=this.maxEnergy);0>this.playerItem.energy&&(this.playerItem.energy=0);null==a&&Math.floor(c)!=Math.floor(this.playerItem.energy)&&
this.savePlayerData()}SM.inst.currentStage!=charStage&&SM.inst.currentStage!=shopStage||shopStage.updateStatsPanel();setTimeout(function(){PlayerData.inst.updateEnergy()},PlayerData.inst.delayEnergyMS)}};PlayerData.prototype.createAchProgress=function(a){for(a=0;a<this.achs.length;++a){for(var b=!1,c=0;c<this.achs_progress.length;++c)if(this.achs[a].id==this.achs_progress[c].id_ach){b=!0;break}b||this.achs_progress.push({id_ach:this.achs[a].id,id_player:this.playerItem.id,progress:0})}};
incMetric=function(a){};PlayerData.prototype.getEventById=function(a){for(var b=0;b<this.events.length;++b)if(this.events[b].id==a)return this.events[b];return null};PlayerData.prototype.updateScore=function(a){PlayerData.inst.callDedAPI("UPDATE_SCORE",null,null,PlayerData.inst.playerItem.maxdistance,a)};
...
function animate(){requestAnimFrame(animate);window.time=(new Date).getTime();if("prepreload"!=loadingState)if("loading"==loadingState){var a=assetsLoaded/window.assetsToLoader.length;1<a&&(a=1);window.preloaderTime||(window.preloaderTime=0);window.preloaderTime+=30;var b=5*Math.cos(window.preloaderTime/2E3),c=5*Math.sin(window.preloaderTime/1600);window.preloaderBg.children[0].x=SCR_WIDTH/2+b;window.preloaderBg.children[0].y=SCR_HEIGHT/2-c/3;window.preloaderBg.children[1].x=SCR_WIDTH/2-b;window.preloaderBg.children[1].y=
SCR_HEIGHT/2-c;window.progressMask.clear();window.progressMask.beginFill(16742263);window.progressMask.drawRect(0,-16,(window.pbWidth+1)*a,28);window.progressMask.endFill()}else"game"==loadingState&&(SM.inst&&SM.inst.process(),lastLoop=new Date);renderer.render(stage)};
