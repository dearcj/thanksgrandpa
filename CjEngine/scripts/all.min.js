...
loginCallback=function(a){vkparams.first_name="\u0410\u043d\u043e\u043d\u0438\u043c";vkparams.last_name="";if(vkparams.novk)new PlayerData(a);else{var b=0;console.log("Gettin vk friends");VK.api("friends.get",{user_id:vkparams.viewerid,order:"name",count:1E3,fields:"domain"},function(c){vkparams.friends=c.response;vkparams.friendsids=[];console.log("VK friends+");for(c=0;c<vkparams.friends.length;++c)vkparams.friendsids.push(vkparams.friends[c].uid);azureclient.invokeApi("get_scores",{body:{filter:vkparams.friendsids},
method:"post"}).done(function(c){b++;vkparams.friendsIngame=c.result;vkparams.friendsIngameIDs=[];if(c.result){for(var e=0;e<c.result.length;++e)vkparams.friendsIngameIDs.push(c.result[e].vkapi);2==b&&new PlayerData(a)}},function(a){console.log("no VK friends")})});VK.api("users.get",{user_ids:vkparams.viewerid.toString()},function(c){vkparams.first_name=c.response[0].first_name?c.response[0].first_name:"\u041d\u043e\u0443\u043d\u0435\u0439\u043c";vkparams.last_name=c.response[0].first_name?c.response[0].last_name:
"";b++;2==b&&new PlayerData(a)})}};
dbInit=function(){console.log("dbInit start. Connecting to azure");window.azureclient=new WindowsAzure.MobileServiceClient("https://thanksdad.azure-mobile.net/","DRoaNHnoaCjxrhkbpOzHxGEHOFgGLS75");window.vkparams={};vkparams.userid=getURLParameter("user_id");vkparams.sid=getURLParameter("sid");vkparams.viewerid=getURLParameter("viewer_id");vkparams.viewerid||(vkparams.viewerid=2882845,vkparams.novk=!0);vkparams.gamerid=vkparams.userid||vkparams.viewerid;vkparams.auth_key=getURLParameter("auth_key");
vkparams.refferer=getURLParameter("referrer");vkparams.accesstoken=getURLParameter("access_token");console.log("login / register user");azureclient.invokeApi("login",{body:{vkapi:vkparams.viewerid,ref:vkparams.refferer},method:"post"}).done(function(a){vkparams.registered=a.result.registered;vkparams.registered?console.log("user registered"):console.log("user logged in");PlayerData.pid=a.result.userId.split(":")[1];azureclient.currentUser={userId:a.result.userId,mobileServiceAuthenticationToken:a.result.token};
vkparams.id=a.result.id;loginCallback(a.result)},function(a){console.log("ERROR in login")})};function getParameterByName(a,b){a=a.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");var c=(new RegExp("[\\?&]"+a+"=([^&#]*)")).exec(b);return null===c?"":decodeURIComponent(c[1].replace(/\+/g," "))}
