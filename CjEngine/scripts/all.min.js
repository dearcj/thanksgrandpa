...
GameStage.prototype.mobileTouchStart=function(a){};GameStage.prototype.mobileTouchEnd=function(a){};
...
PlayerData.getVKfriends=function(a){vkparams.first_name="\u0410\u043d\u043e\u043d\u0438\u043c";vkparams.last_name="";vkparams.novk?new PlayerData(a):(console.log("Gettin vk friends"),VK.api("friends.getAppUsers",{},function(b){b.response&&b.response.length?(console.log("friends.get data"+JSON.stringify(b.response)),vkparams.friends=b.response,vkparams.friendsids=b.response,console.log("VK friends+")):vkparams.friendsids=[];azureclient.invokeApi("get_scores",{body:{filter:vkparams.friendsids,take:1E3},
method:"post"}).done(function(b){vkparams.friendsIngame=b.result;vkparams.friendsIngameIDs=[];if(b.result){for(var d=0;d<b.result.length;++d)vkparams.friendsIngameIDs.push(b.result[d].vkapi);PlayerData.getVKuserData(a)}},function(b){PlayerData.getVKuserData(a)})}))};
...
2;a.y=SCR_HEIGHT/2+160;preloaderBg.addChild(a);SM.inst.addLayersToStage();SM.inst.superStage.addChild(preloaderBg);a=crsp("loading bar line");a.x=SCR_WIDTH/2;a.y=SCR_HEIGHT/2+160;window.pbWidth=a.width;a.mask=new PIXI.Graphics;a.mask.x=-pbWidth/2;window.progressMask=a.mask;a.addChild(a.mask);preloaderBg.addChild(a);SM.inst.addLayersToStage();SM.inst.superStage.addChild(loadingScreen);window.loadingState="loading";window.assetsToLoader="levels/energywindow.json levels/settings.json levels/levelpremium.json levels/levelup.json levels/levach.json levels/levshop.json levels/hud.json levels/plantPart1.json levels/plantPart2.json levels/levelmenu.json levels/levelwin.json levels/levelgameover.json levels/rotatescreen.json levels/levchar.json levels/levscore.json levels/upperPanel.json imgtps/comix.json imgtps/bg.json imgtps/guiatlas.json imgtps/pussyatlas.json imgtps/dedgamedesc.xml imgtps/dedgamecaps.xml imgtps/dedgameXP.xml imgtps/skeleton.json imgtps/boss1.json imgtps/bird.json".split(" ");
window.prevW=window.innerWidth;window.prevH=window.innerHeight;window.loader=new PIXI.AssetLoader(assetsToLoader);document.body.appendChild(renderer.view);PIXI.scaleModes.DEFAULT=0;window.charStage=new CharStage;window.comixStage=new ComixStage;window.scoreStage=new ScoreStage;window.gameStage=new GameStage;window.achStage=new AchStage;window.shopStage=new ShopStage;requestAnimFrame(animate);window.pool=new ZPool;FRAME_DELAY=1E3/FRAME_RATE;window.addEventListener("resize",onWindowResize,!1);window.addEventListener("resizeend",
...
function onAssetsLoaded(){if(window.dbinit&&1==ZSound.loaded&&window.gfxLoaded){rp(preloaderBg);loadingState="game";rp(loadingScreen);preloaderBg=loadingScreen=null;PIXI.TextureCache["1st plan.png"].destroy(!0);window.progressMask=null;window.addEventListener("orientationchange",orientchange,!1);orientchange();setTimeout("window.scrollTo(0, 1)",10);var a=document.getElementById("vk_ads_55316");a&&a.parentNode.removeChild(a);PlayerData.inst.updateEnergy();0==PlayerData.inst.playerItem.maxdistance?
