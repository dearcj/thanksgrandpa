...
function superRand(a){for(var b=0,c=0;c<a;++c)b+=Math.random();return b/a}function dateDiff(a,b,c){a=(new Date).getTime()-a.getTime();a=(6E4*b-a)/1E3;b=Math.floor(a/3600);a%=3600;var d=Math.floor(a/60);a%=60;var e=Math.floor(a%60);c=(c?"":(10>b?"0"+b:b)+" : ")+((10>d?"0"+d:d)+" : "+(10>e?"0"+e:e));return{d:a,m:d,h:b,s:e,timeString:c}}function crsp(a){var b=4<=a.length&&"."!=a.charAt(a.length-4)?".png":"";a=new PIXI.Sprite(PIXI.Texture.fromFrame(a+b));a.anchor.x=.5;a.anchor.y=.5;return a}
...
gameStage.makePause,SM.inst.fontLayer))};var b=gameStage.doKeyDown,c=gameStage.doKeyUp;$(function(){$(document).on("keydown",b);$(document).on("keyup",c);$(document).mousedown(gameStage.fdown);$(document).mouseup(gameStage.fup)});MOBILE&&$(function(){$(document).on("touchstart",gameStage.mobileTouchStart);$(document).on("touchend",gameStage.mobileTouchEnd)});gameStage.updateXP();LauncherBG.inst.maxVelocity=.1;new TweenMax(LauncherBG.inst,3,{maxVelocity:10,ease:Sine.easeIn});gameStage.tutorial&&!gameStage.tutComplete?
gameStage.showTutorial():PlayerData.inst.progressAch("Gold medal 1",1,!1)};GameStage.prototype.makePause=function(){removePause=function(){gameStage.state="game";gameStage.doProcess=!0;LevelManager.removeLastLevel();LevelManager.objs=null;gameStage.visibleAllText(!0);gameStage.doPhys=!0;gameStage.unpause();gameStage.removeFade()};charStage.updateMusicButton(CObj.getById("bmute"));CObj.getById("bresume").click=function(){removePause()};CObj.getById("blevs").click=function(){removePause();SM.inst.openStage(charStage)}};
