...
uploadPhoto=function(a){var b=new PIXI.RenderTexture(15,15);b.render(stage);renderer.render(stage);var c=b.getBase64(),c=c.replace(/^data:image\/(png|jpg);base64,/,"");VK.api("photos.getWallUploadServer",{uid:a},function(a){$.ajax({type:"POST",url:"upload.php",data:{uploadUrl:a.response.upload_url,photo:c},dataType:"text"}).success(function(a){a=JSON.parse(a);VK.api("wall.savePost",{wall_id:VK.params.viewer_id,server:a.server,photo:a.photo,hash:a.hash,message:"\u041a\u0443-\u041a\u0443"},function(a){a.response&&
(VK.addCallback("onWallPostSave",app.onWallPost),VK.addCallback("onWallPostCancel",app.onWallPost),VK.callMethod("saveWallPost",a.response.post_hash))});console.log("")}).error(function(a){console.log("")})})};
