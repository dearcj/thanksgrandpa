...
PlayerData.getVKfriends=function(a){vkparams.first_name="\u0410\u043d\u043e\u043d\u0438\u043c";vkparams.last_name="";vkparams.novk?new PlayerData(a):(console.log("Gettin vk friends"),VK.api("friends.get",{user_id:vkparams.viewerid,order:"name",count:1E3,fields:"domain"},function(b){if(b.response&&b.response.length){vkparams.friends=b.response;vkparams.friendsids=[];console.log("VK friends+");for(b=0;b<vkparams.friends.length;++b)vkparams.friendsids.push(vkparams.friends[b].uid);azureclient.invokeApi("get_scores",
{body:{filter:vkparams.friendsids},method:"post"}).done(function(b){vkparams.friendsIngame=b.result;vkparams.friendsIngameIDs=[];if(b.result){for(var d=0;d<b.result.length;++d)vkparams.friendsIngameIDs.push(b.result[d].vkapi);PlayerData.getVKuserData(a)}},function(a){})}else PlayerData.getVKfriends(a)}))};
...
vkparams.id=a.result.id;PlayerData.getVKfriends(a.result)},function(a){console.log("ERROR in login")})};var tWeapon="weap",tPerk="perk",tBoost="boost",tApp="app.",tHat="hat";function checkDb(){updDb(dbobj)}
