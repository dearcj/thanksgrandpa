...
function clone(a){return JSON.parse(JSON.stringify(a))}function superRand(a){for(var b=0,c=0;c<a;++c)b+=Math.random();return b/a}function dateDiff(a,b,c){a=(new Date).getTime()-a.getTime();a=6E4*b-a;0>a&&(a=0);a/=1E3;b=Math.floor(a/3600);a%=3600;var d=Math.floor(a/60);a%=60;var e=Math.floor(a%60);c=(c?"":(10>b?"0"+b:b)+" : ")+((10>d?"0"+d:d)+" : "+(10>e?"0"+e:e));return{d:a,m:d,h:b,s:e,timeString:c}}
...
PlayerData.dbInit=function(){console.log("dbInit start. Connecting to azure");window.azureclient=new WindowsAzure.MobileServiceClient("https://thanksdad.azure-mobile.net/","DRoaNHnoaCjxrhkbpOzHxGEHOFgGLS75");window.vkparams={};vkparams.userid=getURLParameter("user_id");vkparams.sid=getURLParameter("sid");vkparams.viewerid=getURLParameter("viewer_id");vkparams.viewerid&&VK||(vkparams.viewerid="RANDOM0000004",MOBILE&&(vkparams.viewerid=Cocoon.Device.getDeviceId()),vkparams.novk=!0);vkparams.gamerid=
vkparams.userid||vkparams.viewerid;vkparams.auth_key=getURLParameter("auth_key");vkparams.refferer=getURLParameter("referrer");vkparams.accesstoken=getURLParameter("access_token");console.log("login / register user");PlayerData.azureLogin()};var tWeapon="weap",tPerk="perk",tBoost="boost",tApp="app.",tHat="hat";function checkDb(){}
