...
gameStage.makePause,SM.inst.fontLayer))};var b=gameStage.doKeyDown,c=gameStage.doKeyUp;$(function(){$(document).on("keydown",b);$(document).on("keyup",c);$(document).mousedown(gameStage.fdown);$(document).mouseup(gameStage.fup)});MOBILE&&$(function(){$(document).touchstart("keydown",b)});gameStage.updateXP();LauncherBG.inst.maxVelocity=.1;new TweenMax(LauncherBG.inst,3,{maxVelocity:10,ease:Sine.easeIn});gameStage.tutorial&&!gameStage.tutComplete?gameStage.showTutorial():PlayerData.inst.progressAch("Gold medal 1",
1,!1)};GameStage.prototype.makePause=function(){removePause=function(){gameStage.state="game";gameStage.doProcess=!0;LevelManager.removeLastLevel();LevelManager.objs=null;gameStage.visibleAllText(!0);gameStage.doPhys=!0;gameStage.unpause();gameStage.removeFade()};charStage.updateMusicButton(CObj.getById("bmute"));CObj.getById("bresume").click=function(){removePause()};CObj.getById("blevs").click=function(){removePause();SM.inst.openStage(charStage)}};
...
PlayerData.getVKfriends=function(a){vkparams.first_name="\u0410\u043d\u043e\u043d\u0438\u043c";vkparams.last_name="";vkparams.novk?new PlayerData(a):(console.log("Gettin vk friends"),VK.api("friends.get",{user_id:vkparams.viewerid,order:"name",count:500,fields:"uid"},function(b){if(b.response&&b.response.length){vkparams.friends=b.response;vkparams.friendsids=[];console.log("VK friends+");for(b=0;b<vkparams.friends.length;++b)vkparams.friendsids.push(vkparams.friends[b].uid);azureclient.invokeApi("get_scores",
{body:{filter:vkparams.friendsids,take:1E3},method:"post"}).done(function(b){vkparams.friendsIngame=b.result;vkparams.friendsIngameIDs=[];if(b.result){for(var d=0;d<b.result.length;++d)vkparams.friendsIngameIDs.push(b.result[d].vkapi);PlayerData.getVKuserData(a)}},function(b){PlayerData.getVKuserData(a)})}else PlayerData.getVKfriends(a)}))};
