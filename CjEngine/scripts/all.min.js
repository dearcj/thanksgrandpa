...
LevelManager.loadLevel=function(a,b,c){LevelManager.objs=CObj.DeserializeArray(LevelManager.levels[LevelManager.levFolder+a+".json"]);LevelManager.layer=c;LevelManager.objs.sort(function(a,b){return a.layer==b.layer?0:a.layer<b.layer?-1:1});a=[];for(c=0;c<LevelManager.objs.length;++c)void 0!=LevelManager.objs[c].sAtlasName&&""!=LevelManager.objs[c].sAtlasName&&0>a.indexOf(LevelManager.objs[c].sAtlasName)&&a.push(LevelManager.objs[c].sAtlasName);LevelManager.onVeryComplete=b;b=[];for(c=0;c<a.length;++c){var d=
...
!0),LevelManager.loadLevel("levelmenu",gameStage.makePause,SM.inst.fontLayer))};var b=gameStage.doKeyDown,c=gameStage.doKeyUp;$(function(){$(document).on("keydown",b);$(document).on("keyup",c);$(document).mousedown(gameStage.fdown);$(document).mouseup(gameStage.fup)});MOBILE&&$(function(){$(document).on("touchstart",gameStage.mobileTouchStart);$(document).on("touchend",gameStage.mobileTouchEnd)});gameStage.updateXP();LauncherBG.inst.maxVelocity=.1;new TweenMax(LauncherBG.inst,3,{maxVelocity:10,ease:Sine.easeIn});
gameStage.tutorial&&!gameStage.tutComplete?gameStage.showTutorial():PlayerData.inst.progressAch("Gold medal 1",1,!1)};
...
function(a){var b=CObj.getById("energybg");b.gfx.getBounds();window.mouseX=a.global.x/SCR_SCALE;window.mouseY=a.global.y/SCR_SCALE;window.mouseX>b.x-b.gfx.width/2&&window.mouseX<b.x+b.gfx.width/2&&window.mouseY>b.y-b.gfx.height/2&&window.mouseY<b.y+b.gfx.height/2||close()})},SM.inst.fontLayer)};
...
charStage.createFriendsPanel()},CObj.getById("frnext").click=function(){charStage.skipFriends+=5;charStage.frp.parent.removeChild(charStage.frp);charStage.frp=charStage.createFriendsPanel()});CObj.getById("bbuy1").click=charStage.openPremiumWindow;CObj.getById("bbuy2").click=charStage.openPremiumWindow;CObj.getById("tname").text=vkparams.first_name.toUpperCase()+" "+vkparams.last_name.toUpperCase();CObj.getById("bpubl").click=function(){OpenInNewTab("https://vk.com/thanksgradpa")};charStage.updateMusicButton(CObj.getById("bmusic"));
CObj.getById("bshop").click=function(){SM.inst.openStage(shopStage)};CObj.getById("bscore").click=function(){charStage.openScore()};CObj.getById("btnachs").click=function(){SM.inst.openStage(achStage)};CObj.getById("btnfight").click=function(){if(1<=PlayerData.inst.playerItem.energy)--PlayerData.inst.playerItem.energy,PlayerData.inst.savePlayerData(),SM.inst.openStage(gameStage);else{var a=CObj.getById("tfenergy"),c=CObj.getById("energyback");new TweenMax(a,.1,{y:a.y-10,repeat:3,yoyo:!0,ease:Linear.easeInOut});
new TweenMax(c,.1,{y:c.y-10,repeat:3,yoyo:!0,ease:Linear.easeInOut});charStage.openEnergyWindow()}};var a=new CPlayer(SCR_WIDTH/2,430);charStage.pl=a;a.gfx.scale.x=.34;a.gfx.scale.y=.34;a.updateAppearence(!0,!1,"breath");SM.inst.ol.addChild(a.gfx);a.gfx.interactive=!0;CObj.getById("bsofa").click=function(){CObj.enableButtons(!1);charStage.disableWnd=SM.inst.addDisableWindow(null,SM.inst.fontLayer);charStage.disableWnd.interactive=!0;for(var a=0;a<CObj.objects.length;++a)CObj.checkType(CObj.objects[a],
CEActionGUI)&&CObj.objects[a].btnReward&&(CObj.objects[a].btnReward.gfx.interactive=!0);charStage.closeEventsWnd=function(){rp(charStage.disableWnd);noc(charStage.disableWnd);CObj.enableButtons(!0);charStage.bar.gfx.visible=!1};onc(charStage.disableWnd,function(a){var b=charStage.bar;window.mouseX=a.global.x/SCR_SCALE;window.mouseY=a.global.y/SCR_SCALE;window.mouseX>b.x-b.gfx.width/2&&window.mouseX<b.x+b.gfx.width/2&&window.mouseY>b.y-b.gfx.height/2&&window.mouseY<b.y+b.gfx.height/2||charStage.closeEventsWnd()});
charStage.bar.gfx.parent.removeChild(charStage.bar.gfx);SM.inst.fontLayer.addChild(charStage.bar.gfx);charStage.bar.gfx.visible=!0;charStage.unreadActions=!1;charStage.updateNotifications()};PlayerData.inst.comboCheck();charStage.bar=new CScrollbar(180,309,"",380,524,"podlozhka actions.png","scroll line actions.png","scroll.png",50);charStage.bar.gfx.scale.x=.7;charStage.bar.gfx.scale.y=.7;charStage.bar.gfx.visible=!1;charStage.updateEvents();shopStage.updateStatsPanel();MOBILE||(charStage.frp=charStage.createFriendsPanel())};
...
fontSize:18,align:"center"});this.rewText.x=80;this.rewText.y=-12;this.gfx.addChild(this.rewText);this.timeleft=CTextField.createTextField({tint:"0x333333",text:"",fontSize:17,align:"center"});this.timeleft.x=80;this.timeleft.y=27;this.gfx.addChild(this.timeleft);c=CTextField.createTextField({tint:"0x333333",text:c.toString(),fontSize:16,align:"center"});c.x=200;c.y=-11;this.gfx.addChild(c);var f=this,g=this.ico;g.interactive=!0;var k=g.scale.x,h=g.scale.y;g.mouseover=function(){new TweenMax(g.scale,
...
this;this.loadData(this.loadEnd)};PlayerData.prototype.addNotification=function(a,b){window.azureclient.getTable("tb_notifications").insert({message:a,vkapi:b})};
...
PauseTimer.isPaused=function(){return PauseTimer.paused};PauseTimer.pause=function(){PauseTimer.pauseStartedAt=window.time;TweenMax.pauseAll();PauseTimer.paused=!0};PauseTimer.resume=function(){PauseTimer.paused&&(PauseTimer.totalPauseTime+=window.time-PauseTimer.pauseStartedAt,TweenMax.resumeAll(),PauseTimer.paused=!1)};window.MOBILE?(window.SCR_WIDTH=1024,window.SCR_HEIGHT=576):(window.SCR_WIDTH=800,window.SCR_HEIGHT=600);window.SCR_SCALE=1;window.FRAME_RATE=60;
function isSafari(){return/^((?!chrome).)*safari/i.test(navigator.userAgent)}var loadingState="prepreload";window.openSponsorWindow=null;window.focus();var assetsLoaded=0,preloaderAsset=["imgtps/preloader.json"];window.addScale=1;window.renderer=new PIXI.autoDetectRenderer(window.SCR_WIDTH,window.SCR_HEIGHT);window.loader=new PIXI.AssetLoader(preloaderAsset);window.loader.onComplete=preloaderLoaded;window.loader.load();PlayerData.dbInit();$(document).bind("contextmenu",function(){return!1});
window.apiid=4654201;VK&&VK.init({apiId:window.apiid},function(){},function(){});function order(a){var b={type:"item",item:a};window.currentOrder=a;VK.callMethod("showOrderBox",b)}
...
"levchar.json",LevelManager.levFolder+"levscore.json",LevelManager.levFolder+"upperPanel.json","imgtps/comix.json","imgtps/bg.json","imgtps/guiatlas.json","imgtps/pussyatlas.json","imgtps/dedgamedesc.xml","imgtps/dedgamecaps.xml","imgtps/dedgameXP.xml","imgtps/skeleton.json","imgtps/boss1.json","imgtps/bird.json"];window.prevW=window.innerWidth;window.prevH=window.innerHeight;window.loader=new PIXI.AssetLoader(assetsToLoader);document.body.appendChild(renderer.view);PIXI.scaleModes.DEFAULT=0;window.charStage=
new CharStage;window.comixStage=new ComixStage;window.scoreStage=new ScoreStage;window.gameStage=new GameStage;window.achStage=new AchStage;window.shopStage=new ShopStage;requestAnimFrame(animate);window.pool=new ZPool;FRAME_DELAY=1E3/FRAME_RATE;window.addEventListener("resize",onWindowResize,!1);window.addEventListener("resizeend",onWindowResize,!1);rescale();onWindowResize();window.lastLoop=0;assetsButSoundsLoaded();loader.onComplete=function(){window.loaded=!0;console.log("gfx loaded");window.gfxLoaded=
!0;onAssetsLoaded()};loader.load();loader.onProgress=onAssetsProgress}function onAssetsProgress(a){console.log(window.assetsLoaded.toString()+" asset loaded");window.assetsLoaded++;a.json&&void 0!=a.json.objects&&(LevelManager.levels[a.url]=a.json)}
